...
var createDefaultTestApp = function(globalAdminRestClient, tenant, hostSuffix, year, callback) {
    ...
    globalAdminRestClient.app.createApp(displayName, host, tenant.id, 'timetable', function(err, app) {
        if (err) {
            return callback(err);
        }

        // Create an app administrator for this app
        generateTestUser(app, function(err, user) {
            if (err) {
                return callback(err);
            }
...
/**
 * Get a REST client that is authenticated as a global administrator
 *
 * @param  {Function}       callback                    Standard callback function
 * @param  {RestClient}     callback.client             A global administrator REST client
 */
 ...
 */
var generateTestUsers = module.exports.generateTestUsers = function(app, total, callback, _createdUsers) {
    ...
    generateTestUser(app, function(err, user) {
        ...
        return generateTestUsers(app, total - 1, callback, _createdUsers);
    });
};
 ...
 *
 * @param  {RestClient}     app                         The app on which to create the test user
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {Object}         callback.user               An object containing the REST client and profile of the created user
 * @param  {User}           callback.user.profile       The basic user profile of the created user
 * @param  {RestClient}     callback.user.client        The REST client for the created user
 */
var generateTestUser = function(app, callback) {
    ...
    getGlobalAdminRestClient(function(globalAdminClient) {
        globalAdminClient.user.createUser(displayName, email, password, {'appId': app.id}, function(err, user) {
            if (err) {
                return callback(err);
            }
