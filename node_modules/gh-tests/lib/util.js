 ...
 * @param  {String}             appName                 The name of the application from which the calendar is retrieved
 * @param  {String}             calendarName            The name of the user or object that owns the calendar
 * @param  {Function}           callback                Standard callback function
 * @param  {Object}             callback.calendar       The parsed iCal calendar
 * @throws {AssertionError}                             Error thrown when an assertion failed
 */
var parseIcalCalendar = module.exports.parseIcalCalendar = function(calendar, appName, calendarName, callback) {
    var parser = new ICalParser();
    parser.parseString(calendar, function(err, calendar) {
        assert.ok(!err);

        // Assert we return a name for this calendar
        var prodId = _.find(calendar.rawFields, {'key': 'PRODID'});
        assert.ok(prodId);
        assert.ok(prodId.value);
        assert.strictEqual(prodId.value, util.format('-//%s//NONSGML %s//EN', appName, calendarName));

        var calName = _.find(calendar.rawFields, {'key': 'X-WR-CALNAME'});
        assert.ok(calName);
        assert.ok(calName.value);
        assert.strictEqual(calName.value, appName + ' - ' + calendarName);
