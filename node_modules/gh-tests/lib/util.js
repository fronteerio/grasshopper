 ...
 *
 * @return {Object}    config    JSON object containing the application configuration
 */
...
var createDefaultTestApp = function(globalAdminRestClient, tenant, hostSuffix, year, callback) {
    ...
    globalAdminRestClient.app.createApp(app, function(err, app) {
        if (err) {
            return callback(err);
        }
 ...
 *
 * @param  {RestClient}     restclient          A rest client connected to an application on whose tenant we should generate test users
 ...
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.user1      An object with key `profile` containing the User object, and `restClient` containing that user's rest client for one of the users that was created
 ...
 * Generate a number of random users that can be used inside of tests
 *
 * @param  {RestClient}     restclient                  A rest client. If no `tenantId` is specified, the tenant of the application the client is connect to will be used to generate the test user on
 * @param  {Number}         total                       The total number of test users that need to be created. If not provided, a single test user will be created
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.user1              An object with the profile and rest client for the first user
 * @param  {User}           callback.user1.profile      The basic user profile of this user
 * @param  {RestClient}     callback.user1.client       A rest client that is authenticated for this user
 * @param  {Object}         [callback.user2]            Another user that was created
 * @param  {Object}         [callback....]              Each user that was generated as new callback arguments
 */
var generateTestUser = function(restClient, tenantId, host, hostHeader, callback) {
    ...
    var displayName = generateTestUserId('random-user');
    var opts = {'tenantId': tenantId};
    restClient.user.create(username, 'password', displayName, email, opts, function(err, profile) {
        if (err) {
            return callback(err);
        }
        ...
        options.authenticationStrategy = 'local';
