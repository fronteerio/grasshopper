...
describe('Organisational units - export', function() {
    ...
    it('verify exporting a tree with borrowed series', function(callback) {
        // Create a tree
        OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'root', 'course', null, function(root) {
            OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'A', 'part', {'parent': root.id}, function(partA) {
                OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'B', 'part', {'parent': root.id}, function(partB) {

                    // Create a series and stick it under both parts
                    var start = moment().format();
                    var end = moment().add(2, 'hour').format();
                    SeriesTestUtil.generateSerieWithEvents(global.tests.admins.cam2013.client, 1, 5, start, end, function(series) {
                        series = series[0];
                        OrgUnitTestUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, partA.id, [series.id], function() {
                            OrgUnitTestUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, partB.id, [series.id], function() {

                                // Export the root node
                                OrgUnitTestUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, root.id, null, function(data) {
        ...
        OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'root', 'course', null, function(root) {
            ...
            OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'A', 'part', {'parent': root.id}, function(partA) {
                OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'B', 'part', {'parent': root.id}, function(partB) {
                    ...
                    OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'B-1', 'module', {'parent': partB.id}, function(module) {

                        // Create a test serie and event
                        SeriesTestUtil.assertCreateSerie(global.tests.admins.cam2013.client, 'Test serie', null, function(series) {
                            var start = moment().format();
                            var end = moment().add(2, 'hour').format();
                            EventsTestsUtil.assertCreateEvent(global.tests.admins.cam2013.client, 'Test event', start, end, {'series': series.id}, function(event) {
                                ...
                                OrgUnitTestUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, partA.id, [series.id], function() {
                                    OrgUnitTestUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, module.id, [series.id], function() {
                                        ...
                                        OrgUnitTestUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, root.id, null, function(data) {
                                            ...
                                            OrgUnitTestUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, root.id, 'csv', function(records) {
                                                ...
                                                // Expected keys
                                                var expectedColumns = ['course_id', 'course_externalId', 'course_displayName', 'course_description', 'course_published', 'part_id', 'part_externalId', 'part_displayName', 'part_description', 'part_published', 'module_id', 'module_externalId', 'module_displayName', 'module_description', 'module_published', 'series_id', 'series_externalId', 'series_displayName', 'series_description', 'event_id', 'event_externalId', 'event_displayName', 'event_description', 'event_notes', 'event_location', 'event_start', 'event_end', 'organiser_id', 'organiser_shibbolethId', 'organiser_displayName'];
                                                _.each(records, function(record) {
                                                    _.each(record, function(val, key) {
                                                        assert.ok(_.contains(expectedColumns, key));
                                                    });
                                                });
                                                ...
                                                records = _.partition(records, function(record) {
                                                    return (record.module_id === '' + module.id);
                                                });
                                                ...
                                                assert.strictEqual(partRecord.module_id, '');
                                                assert.strictEqual(partRecord.module_displayName, '');
                                                assert.strictEqual(partRecord.module_description, '');
                                                assert.strictEqual(partRecord.module_externalId, '');
    ...
    it('verify exporting an organisational unit structure with a cycle in it', function(callback) {
        OrgUnitTestUtil.createSimpleTree(global.tests.admins.cam2013.client, function(course, subject, part, module, series) {
            ...
            OrgUnitTestUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, course.id, null, function(data) {
                ...
                OrgUnitDAO.getOrgUnit(course.id, false, function(err, courseOrgUnit) {
                    ...
                    OrgUnitDAO.updateOrgUnit(courseOrgUnit, {'ParentId': module.id}, function(err) {
                        ...
                        OrgUnitTestUtil.assertExportOrgUnitFails(global.tests.admins.cam2013.client, course.id, null, 400, function() {
                            return callback();
                        });
                    });
                });
    ...
    it('verify organisational units without children or series without events are included in the export', function(callback) {
        ...
        OrgUnitTestUtil.createSimpleTree(global.tests.admins.cam2013.client, function(course, subject, part, module, series) {
            ...
            OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, TestsUtil.generateString(20), 'subject', {'parent': course.id}, function(noChildrenOrSeries) {
                // Create and add an organisational unit that will hold an empty series
                OrgUnitTestUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, TestsUtil.generateString(20), 'subject', {'parent': course.id}, function(emptySeriesParent) {
                    SeriesTestUtil.assertCreateSerie(global.tests.admins.cam2013.client, TestsUtil.generateString(20), null, function(series) {
                        OrgUnitTestUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, emptySeriesParent.id, [series.id], function() {
                            ...
                            OrgUnitTestUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, course.id, null, function(data) {
                                ...
                                OrgUnitTestUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, course.id, 'csv', function(data) {

                                    // The course should have 3 children
                                    var uniqueSubjectIds = _.chain(data).pluck('subject_id').uniq().value();
                                    assert.strictEqual(uniqueSubjectIds.length, 3);

                                    // Assert the organisational unit without any children or series has been exported
                                    var exportedNoChildrenOrSeriesOrgUnit = _.find(data, {'subject_id': '' + noChildrenOrSeries.id});
                                    assert.ok(exportedNoChildrenOrSeriesOrgUnit);
                                    assert.strictEqual(exportedNoChildrenOrSeriesOrgUnit.subject_displayName, noChildrenOrSeries.displayName);

                                    // Assert the series without any events was exported
                                    var exportedEmptySeriesParent = _.find(data, {'subject_id': '' + emptySeriesParent.id});
                                    assert.ok(exportedEmptySeriesParent);
                                    assert.strictEqual(exportedEmptySeriesParent.subject_displayName, emptySeriesParent.displayName);
