...
describe('Organisational units - export', function() {
    ...
    it('verify exporting an unbalanced tree', function(callback) {
        TestsUtil.generateTestUsers(global.tests.apps.cam2013, 1, false, function(nico) {
            OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'root', 'course', null, function(root) {
                ...
                OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'A', 'part', {'parent': root.id, 'description': 'A'}, function(partA) {
                    OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'B', 'part', {'parent': root.id, 'description': 'B'}, function(partB) {

                        // Add another organisational unit under part B
                        OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'B-1', 'module', {'parent': partB.id, 'description': 'B-1'}, function(module) {
                            ...
                            SeriesTestsUtil.assertCreateSerie(global.tests.admins.cam2013.client, 'Test serie', null, function(series) {
                                var start = moment().format();
                                var end = moment().add(2, 'hour').format();
                                EventsTestsUtil.assertCreateEvent(global.tests.admins.cam2013.client, 'Test event', start, end, {'series': series.id, 'organiserUsers': [nico.profile.id]}, function(event) {
                                        ...
                                        OrgUnitTestsUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, module.id, [series.id], function() {
                                            ...
                                            OrgUnitTestsUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, root.id, null, function(data) {
                                                ...
                                                OrgUnitTestsUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, root.id, 'csv', function(records) {
                                                    ...
                                                    // Expected keys
                                                    var expectedColumns = [
                                                        '0_course_id', '0_course_externalId', '0_course_displayName', '0_course_description', '0_course_published', '0_course_parentId',
                                                        '1_part_id', '1_part_externalId', '1_part_displayName', '1_part_description', '1_part_published', '1_part_parentId',
                                                        '2_module_id', '2_module_externalId', '2_module_displayName', '2_module_description', '2_module_published', '2_module_parentId',
                                                        'series_id', 'series_externalId', 'series_displayName', 'series_description',
                                                        'event_id', 'event_externalId', 'event_displayName', 'event_description', 'event_type', 'event_notes', 'event_location', 'event_start', 'event_end',
                                                        'organiser_id', 'organiser_shibbolethId', 'organiser_displayName'
                                                    ].sort();
                                                    ...
                                                    records = _.partition(records, function(record) {
                                                        return (record['2_module_id'] === '' + module.id);
                                                    });
                                                    ...
                                                    // The module record's metadata should be present
                                                    assert.strictEqual(moduleRecord['2_module_id'], '' + module.id);
                                                    assert.strictEqual(moduleRecord['2_module_displayName'], module.displayName);
                                                    assert.strictEqual(moduleRecord['2_module_description'], module.description);

                                                    // The part record has no module metadata
                                                    assert.strictEqual(partRecord['2_module_id'], '');
                                                    assert.strictEqual(partRecord['2_module_displayName'], '');
                                                    assert.strictEqual(partRecord['2_module_description'], '');
                                                    assert.strictEqual(partRecord['2_module_externalId'], '');
    ...
    /**
     * Verify an organisational unit structure that has types on multiple levels can be exported
     */
    ...
    /**
     * Test that verifies that borrowed series are indicated as such
     */
    it('verify borrowed series are indicated as such', function(callback) {
        OrgUnitTestsUtil.createSimpleTree(global.tests.admins.cam2013.client, function(course, subject, part, module1) {

            // Create a second module that will hold the series that will be borrowed
            OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'Module 2', 'module', {}, function(module2) {
                SeriesTestsUtil.assertCreateSerie(global.tests.admins.cam2013.client, 'The serie that will be borrowed', null, function(series) {
                    OrgUnitTestsUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, module2.id, [series.id], function() {

                        // Borrow the series from module2 to module1
                        OrgUnitTestsUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, module1.id, [series.id], function() {

                            // Export module1
                            OrgUnitTestsUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, module1.id, null, function(data) {
                                // Find the borrowed series and assert it has a correct `borrowedFrom` object
                                var exportedSeries = _.find(data.series, {'id': series.id});
                                assert.ok(exportedSeries);
                                assert.ok(exportedSeries.borrowedFrom);
                                assert.strictEqual(exportedSeries.borrowedFrom.id, module2.id);
                                assert.strictEqual(exportedSeries.borrowedFrom.displayName, module2.displayName);
                                return callback();
                            });
                        });
                    });
                        ...
                        OrgUnitTestsUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, emptySeriesParent.id, [series.id], function() {
                            ...
                            OrgUnitTestsUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, course.id, null, function(data) {
                                ...
                                OrgUnitTestsUtil.assertExportOrgUnit(global.tests.admins.cam2013.client, course.id, 'csv', function(data) {
                                    ...
                                    // The course should have 3 children
                                    var uniqueSubjectIds = _.chain(data).pluck('1_subject_id').uniq().value();
                                    assert.strictEqual(uniqueSubjectIds.length, 3);

                                    // Assert the organisational unit without any children or series has been exported
                                    var exportedNoChildrenOrSeriesOrgUnit = _.find(data, {'1_subject_id': '' + noChildrenOrSeries.id});
                                    assert.ok(exportedNoChildrenOrSeriesOrgUnit);
                                    assert.strictEqual(exportedNoChildrenOrSeriesOrgUnit['1_subject_displayName'], noChildrenOrSeries.displayName);

                                    // Assert the series without any events was exported
                                    var exportedEmptySeriesParent = _.find(data, {'1_subject_id': '' + emptySeriesParent.id});
                                    assert.ok(exportedEmptySeriesParent);
                                    assert.strictEqual(exportedEmptySeriesParent['1_subject_displayName'], emptySeriesParent.displayName);
