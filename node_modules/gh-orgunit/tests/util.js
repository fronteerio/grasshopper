    ...
    _getExportData(client, id, function(orgUnit, childOrgUnits, allSeries, eventsPerSeriesId) {
        ...
        client.orgUnit.exportOrgUnit(id, 'csv', function(err, data, response) {
            ...
            csv.parse(data, {'columns': true}, function(err, records) {
                ...
                // There should be as many organisational unit id columns as there are different types in the tree
                assert.strictEqual(orgUnitIdColumnNames.length, types.length);

                // Assert the correct data is returned
                _.each(records, function(record) {

                    // Assert the organisational units their metadata
                    _.each(types, function(type) {
                        if (record[type + '_id']) {
                            var expectedOrgUnit = _.find(allOrgUnits, {'id': _.parseInt(record[type + '_id'])});
                            assert.ok(expectedOrgUnit);
                            assert.strictEqual(record[type + '_externalId'], expectedOrgUnit.externalId ? expectedOrgUnit.externalId : '');
                            assert.strictEqual(record[type + '_displayName'], expectedOrgUnit.displayName ? expectedOrgUnit.displayName : '');
                            assert.strictEqual(record[type + '_description'], expectedOrgUnit.description ? expectedOrgUnit.description : '');
                            assert.strictEqual(record[type + '_published'], expectedOrgUnit.published ? '1' : '');
                            assert.strictEqual(record[type + '_ParentId'], expectedOrgUnit.ParentId ? expectedOrgUnit.ParentId.toString() : '');
                        }
                    });

                    if (record.series_id) {
                        assert.ok(_.find(allSeries, {'id': _.parseInt(record.series_id)}));
                    }
                    if (record.event_id) {
                        assert.ok(_.find(allEvents, {'id': _.parseInt(record.event_id)}));
                    }

                    _.each(orgUnitIdColumnNames, function(columnName) {
                        assert.ok(_.find(allOrgUnits), {'id': _.parseInt(record[columnName])});
                    });
                });
