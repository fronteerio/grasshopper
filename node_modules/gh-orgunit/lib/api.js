 ...
 * @param  {Object}         [metadata]          The extra metadata for the organisational unit
 * @param  {String}         [visibility]        The visibility of the organisational unit
 ...
 */
var createOrgUnit = module.exports.createOrgUnit = function(ctx, appId, displayName, type, description, metadata, visibility, groupId, parentId, callback) {
    ...
    validator.check(displayName, {'code': 400, 'msg': 'A displayName can be at most 256 characters long'}).isShortString();
    ...
    if (visibility) {
        validator.check(visibility, {'code': 400, 'msg': 'A valid visibility must be provided'}).isIn(['draft', 'published']);
    }
        ...
        OrgUnitAuthz.canCreateOrgUnit(ctx, appId, parent, function(err, canCreate) {
            ...
            AppsDAO.getApp(appId, function(err, app) {
                ...
                GroupsAPI.getOrCreateGroup(ctx, appId, groupId, function(err, group) {
                    ...
                    OrgUnitDAO.createOrgUnit(appId, displayName, type, description, metadata, visibility, group.id, parentId, callback);
                });
            });
        });
    });
 ...
 * @param  {String}         [opts.visibility]       Updated organisational unit visibility
...
var updateOrgUnit = module.exports.updateOrgUnit = function(ctx, id, opts, callback) {
    ...
    var validKeys = ['displayName', 'type', 'description', 'group', 'parent', 'metadata', 'visibility'];
    ...
    if (opts) {
        if (opts.displayName) {
            ...
            validator.check(opts.displayName, {'code': 400, 'msg': 'A displayName can be at most 256 characters long'}).isShortString();
        ...
        if (opts.visibility) {
            validator.check(opts.visibility, {'code': 400, 'msg': 'A valid parent visibility must be provided'}).isIn(['draft', 'published']);
            update.visibility = opts.visibility;
        }
        if (opts.metadata) {
