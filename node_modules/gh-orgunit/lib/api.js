        ...
        OrgUnitAuthz.canCreateOrgUnit(ctx, appId, parent, function(err, canCreate) {
            ...
            AppsDAO.getApp(appId, function(err, app) {
                ...
                GroupsAPI.getOrCreateGroup(ctx, appId, groupId, function(err, group) {
                    ...
                    return OrgUnitDAO.createOrgUnit(appId, displayName, type, group.id, opts, callback);
                });
            });
        });
    });
...
var _augmentOrgUnits = function(ctx, orgUnits, groups) {
    return _.map(orgUnits, function(orgUnit) {
        ...
        var augmentedOrgUnit = orgUnit.toJSON();
        ...
        augmentedOrgUnit.Series = _.map(orgUnit.Series, function(serie) {
            // Serialize the serie
            var augmentedSerie = serie.toJSON();
            ...
            // Sort the organisational units this series is under based on when they were added to the organisational units.
            // This allows us to check whether the series is borrowed from another organisational unit than the one it's
            // currently under. If this series is not borrowed we won't add anything in the output
            serie.OrgUnits = _.sortBy(serie.OrgUnits, function(orgUnit) {
                return orgUnit.OrgUnitsSeries.createdAt.getTime();
            });
            if (serie.OrgUnits[0].id !== orgUnit.id) {
                augmentedSerie.borrowedFrom = serie.OrgUnits[0];
            }

            // In order to save on serialization costs and bandwidth traffic we don't output the data we don't need
            delete augmentedSerie.OrgUnits;
...
/**
 * Walk the tree under an organisational unit and export all the data. The export will include
 * the organisational unit and its children. Any event series and/or events associated to the
 * organisational unit or its children will be exported as well
 *
 * @param  {Context}        ctx                 Standard context containing the current user and the current app
 * @param  {Number}         id                  The id of the organisational unit to export
 * @param  {Number}         [format]            The format in which the organisational units, event series and events should be exported. Options are `json` or `csv`, defaults to `json`
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error object, if any
 * @param  {Object|String}  callback.data       The exported organisational units, event series and events
 */
var exportOrgUnit = module.exports.exportOrgUnit = function(ctx, id, format, callback) {
    format = format || 'json';
    ...
    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'Only authenticated users can export an organisational unit'}).isLoggedInUser(ctx);
    validator.check(id, {'code': 400, 'msg': 'A valid organisational unit id must be provided'}).isInt();
    validator.check(format, {'code': 400, 'msg': 'A valid format must be provided'}).isIn(['csv', 'json']);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    getOrgUnit(ctx, id, false, function(err, orgUnit) {
        ...
        // Ensure the current user can export the organisational unit
        OrgUnitAuthz.canExportOrgUnit(ctx, orgUnit, function(err, canExport) {
            if (err) {
                return callback(err);
            } else if (!canExport) {
                log().warn({'id': orgUnit.id}, 'Unauthorized attempt at exporting an organisational unit');
                return callback({'code': 401, 'msg': 'You are not allowed to export the organisational unit'});
            }

            OrgUnitDAO.exportOrgUnit(orgUnit, format, callback);
        });
    });
};
