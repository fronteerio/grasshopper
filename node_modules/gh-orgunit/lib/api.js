...
var getOrgUnits = module.exports.getOrgUnits = function(ctx, appId, parentId, types, includeSeries, callback) {
    ...
    _getOptionalOrgUnit(ctx, parentId, function(err, parent) {
        ...
        // If the current user is authenticated, we include whether or not
        // the user is subscribed to the series
        // TODO: Make this depend on an API flag
        if (ctx.user) {
            opts.includeSubscriptions = true;
        ...
        OrgUnitDAO.getOrgUnits(appId, parentId, types, opts, function(err, orgUnits) {
            ...
            if (!ctx.user || _.isEmpty(orgUnits)) {
                var augmentedOrgUnits = _augmentOrgUnits(orgUnits, []);
            ...
            GroupsDAO.getUserGroups(ctx.user.id, 10000, 0, function(err, groups) {
                ...
                var augmentedOrgUnits = _augmentOrgUnits(orgUnits, groups);
                return callback(null, augmentedOrgUnits);
            });
        });
    });
 ...
 * @api private
 */
var _augmentOrgUnits = function(orgUnits, groups) {
