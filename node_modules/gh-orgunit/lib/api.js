 ...
 * @param  {Boolean}        [includeSeries]         Whether to include the event series associated to the organisational units. Defaults to `false`
 * @param  {Boolean|Number} [includePermissions]    Whether to include if the current user can manage the organisational units/series and whether an organisational unit is locked. Defaults to `false`
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error object, if any
...
var getOrgUnits = module.exports.getOrgUnits = function(ctx, appId, parentId, types, includeSeries, includePermissions, callback) {
    ...
    // Convert the given includePermissions to a boolean if it's a boolean, but default it to the same
    // value if it's not. This allows us to determine whether it's a number
    includePermissions = GrasshopperUtil.getBooleanParam(includePermissions, includePermissions);
    ...
    // If no boolean flag was provided, it has to be a number
    if (!_.isBoolean(includePermissions)) {
        validator.check(includePermissions, {'code': 400, 'msg': 'The include permissions flag must either be a boolean or number value'}).isInt();

        // TODO: Only administrators can get the permissions for another user
        // TODO: Needs to be user from same application
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    ...
    _getOptionalOrgUnit(ctx, parentId, function(err, parent) {
        ...
        OrgUnitDAO.getOrgUnits(appId, parentId, types, opts, function(err, orgUnits) {
            ...
            if (!_.isBoolean(includePermissions)) {
                userId = _.parseInt(includePermissions);
            }
            GroupsDAO.getUserGroups(userId, 10000, 0, function(err, groups) {
                if (err) {
                    return callback(err);
                }

                var augmentedOrgUnits = _augmentOrgUnits(ctx, orgUnits, groups, userId);
                return callback(null, augmentedOrgUnits);
            });
        });
    });
 ...
 * @api private
 */
var _augmentOrgUnits = function(ctx, orgUnits, groups, userId) {
    return _.map(orgUnits, function(orgUnit) {
        ...
        var canAdminOrgUnit = (ctx.user && ctx.user.id === userId && ctx.user.canAdmin(orgUnit.AppId));
        ...
        augmentedOrgUnit.Series = _.map(orgUnit.Series, function(serie) {
            ...
            var canAdminSerie = (ctx.user && ctx.user.id === userId && ctx.user.canAdmin(serie.AppId));
