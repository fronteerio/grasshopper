 ...
 */
var importOrgUnit = function(req, res) {
    OrgUnitImport.importOrgUnit(req.ctx, req.params.id, req.body.data, req.body.deleteMissing, function(errors) {
        if (errors) {
            ...
            // that is wrong with the provided data. We provide all the information to the user
            if (_.isArray(errors)) {
                var body = _.map(errors, function(error) {
                    return error.msg;
                }).join('\n');
                var code = 400;
                if (errors.length === 1) {
...
GrassHopper.globalAdminRouter.on('post', '/api/orgunit/:id/import', importOrgUnit);
GrassHopper.appRouter.on('post', '/api/orgunit/:id/import', importOrgUnit);
