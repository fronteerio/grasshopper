...
var getOrgUnits = function(req, res) {
    ...
    OrgUnitAPI.getOrgUnits(req.ctx, appId, req.query.parent, types, req.query.includeSeries, function(err, orgUnits) {
        if (err) {
 ...
 * @QueryParam  {boolean}           from                The timestamp (ISO 8601) from which to get the calendar for the organisation unit
 * @QueryParam  {boolean}           to                  The timestamp (ISO 8601) until which to get the calendar for the organisation unit
 ...
 * @FormParam   {string}            displayName         The name of the organisational unit
 * @FormParam   {string}            type                The organisational unit type (e.g. `tripos`, `part`). Limited to 16 characters
 * @FormParam   {string}            [description]       The description of the organisational unit
 * @FormParam   {number}            [group]             The id of the group that can manage the organisational unit. Defaults to creating a new group with the current user as a member
 * @FormParam   {string}            [parent]            The id of the parent organisational unit
 * @Return      {OrgUnit}                               The created organisational unit
...
var createOrgUnit = function(req, res) {
    ...
    OrgUnitAPI.createOrgUnit(req.ctx, appId, req.body.displayName, req.body.type, req.body.description, req.body.group, req.body.parent, function(err, orgUnit) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
var addOrgUnitEvents = function(req, res) {
    var events = GrassHopperUtil.toArray(req.body.event);
    OrgUnitAPI.addOrgUnitEvents(req.ctx, req.params.id, events, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
