...
var importOrgUnit = module.exports.importOrgUnit = function(ctx, id, data, deleteMissing, callback) {
    ...
    try {
        ...
        log().warn({'id': id}, 'Invalid JSON import data submitted');
    }
    ...
    OrgUnitAPI.getOrgUnit(ctx, id, false, function(err, orgUnit) {
        ...
        OrgUnitAuthz.canUpdateOrgUnit(ctx, orgUnit, function(err, canImport) {
            ...
            getParentIfNecessary(orgUnit, function(err, parent) {
                if (err) {
                    return callback(err);
                }
                ...
                OrgUnitDAO.exportOrgUnit(orgUnit, 'json', function(err, exportedOrgUnit) {
                    ...
                    return OrgUnitDAO.importTree(mergedData, callback);
                });
            });
        });
    });
...
var getParentIfNecessary = function(orgUnit, callback) {
    if (!orgUnit.ParentId) {
        return callback();
    }
    ...
    DB.OrgUnit.find(options).complete(function(err, parent) {
        if (err) {
            return callback(err);
        }
