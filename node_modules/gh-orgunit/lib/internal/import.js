 ...
 * @param  {Boolean}        options.deleteMissing       Whether to delete items from the database that aren't in the import data. Defaults to `false`
 * @param  {Boolean}        options.checkExternalId     Whether the provided `data` should be checked for valid external ids. Defaults to `true`
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error object, if any
 */
var importOrgUnit = module.exports.importOrgUnit = function(ctx, id, data, options, callback) {
    options = _.ghOptional(options, {
        'deleteMissing': false,
    ...
    options.deleteMissing = GrasshopperUtil.getBooleanParam(options.deleteMissing, false);

    // Ensure that the checkExternalId flag is a boolean
    options.checkExternalId = GrasshopperUtil.getBooleanParam(options.checkExternalId, false);
    ...
    try {
        ...
        return callback({'code': 400, 'msg': util.format('A valid data object must be provided: %s', err.message)});
    }
    ...
    validator.check(null, {'code': 400, 'msg': 'A valid deleteMissing flag must be provided'}).isBoolean(options.deleteMissing);
        ...
        OrgUnitAuthz.canUpdateOrgUnit(ctx, orgUnit, function(err, canImport) {
            ...
            _getParentIfNecessary(orgUnit, function(err, parent) {
                ...
                OrgUnitDAO.exportOrgUnit(orgUnit, 'json', function(err, exportedOrgUnit) {
                    ...
                    var mergedData = mergeTrees(exportedOrgUnit, data, options.deleteMissing);

                    // Check the external ids again as extra nodes might've been selected
                    if (options.checkExternalId) {
                        _validateImportDataExternalId(validator, data);
                    }
...
var _getParentIfNecessary = function(orgUnit, callback) {
    ...
    var options = {
        ...
        'include': [
            {'model': DB.OrgUnit, 'as': 'Parent', 'include': []}
        ]
    };
...
var _validateImport = function(validator, tree, parent) {
    ...
    _validateImportOrgUnit(validator, tree, parent, 1, seenTypes);
};
 ...
 * @api private
 */
var _validateImportOrgUnit = function(validator, orgUnit, parent, _currentLevel, _seenTypes) {
    ...
    _.each(orgUnit.children, function(childOrgUnit) {
        _validateImportOrgUnit(validator, childOrgUnit, orgUnit, _currentLevel + 1, _seenTypes);
    });

    // Validate each series within this organisational unit
    _.each(orgUnit.series, function(series) {
        _validateImportSeries(validator, series);
    });
