 ...
 * @param  {Boolean}        [opts.includeSubscriptions]     Whether to include the event series associated to the organisational units. Defaults to `false`
 * @param  {Number}         [opts.userId]                   The id of the user for whom the event series subscriptions need to be checked
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error object, if any
...
var getOrgUnits = module.exports.getOrgUnits = function(appId, parentId, types, opts, callback) {
    ...
    if (opts.includeSeries) {
        options.include = [{'model': DB.Serie, 'required': false}];

        if (opts.includeSubscriptions && opts.userId) {
            options.include[0].include = [{'model': DB.Calendar, 'where': {'UserId': opts.userId}, 'required': false, 'limit': null}];
        }
    }
