 ...
 * @param  {Object}             callback.err        An error object, if any
 */
var importTree = module.exports.importTree = function(mergedTreeData, callback) {
    DB.getSequelize().transaction().complete(function(err, transaction) {
        ...
        _importOrgUnit(mergedTreeData.tree, null, function(err) {
            if (err) {
 ...
 * @api private
 */
var _importOrgUnit = function(orgUnit, parent, callback) {
    var parentId = null;
    if (parent) {
        parentId = parent.id;
        orgUnit.AppId = parent.AppId;
    }

    _updateOrCreateOrgUnit(orgUnit, parent, function(err, createdOrgUnit) {
        if (err) {
        ...
        _importOrgUnitChildren(orgUnit.children, createdOrgUnit, function(err) {
            if (err) {
                return callback(err);
            }

            // Import the series under this organisational unit
            _importOrgUnitSeries(orgUnit.series, createdOrgUnit, function(err) {
                if (err) {
                    return callback(err);
                }
 ...
 * @param  {Object}     parent                      The paren organisational unit to which the organisational unit belongs
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error object, if any
 * @param  {Event}      callback.persistedOrgUnit   The persisted organisational unit
 * @api private
 */
var _updateOrCreateOrgUnit = function(orgUnitToImport, parent, callback) {
    if (orgUnitToImport.id) {
        ...
        return createOrgUnit(orgUnitToImport.AppId, orgUnitToImport.displayName, orgUnitToImport.type, parent.GroupId, opts, callback);
    }
};
 ...
 * @api private
 */
var _importOrgUnitChildren = function(children, parent, callback) {
    children = children || [];
    ...
    _importOrgUnit(child, parent, function(err) {
        if (err) {
            return callback(err);
        }

        _importOrgUnitChildren(children, parent, callback);
    });
};
 ...
 * @api private
 */
var _importOrgUnitSeries = function(series, orgUnit, callback) {
    series = series || [];
    if (_.isEmpty(series)) {
        return callback();
    }
    ...
    _importSeries(seriesToImport, orgUnit, function(err) {
        if (err) {
            return callback(err);
        }

        _importOrgUnitSeries(series, orgUnit, callback);
    });
};
 ...
 * @api private
 */
var _importSeries = function(series, orgUnit, callback) {
    _updateOrCreateSeries(series, orgUnit, function(err, persistedSeries) {
        if (err) {
        ...
        addOrgUnitSeries(orgUnit, persistedSeries, function(err) {
            ...
            return _importSeriesEvents(series.events, persistedSeries, callback);
        });
    });
};
 ...
 * @api private
 */
var _updateOrCreateSeries = function(seriesToImport, orgUnit, callback) {
    ...
    if (seriesToImport.id) {
        ...
        SeriesDAO.getSerie(seriesToImport.id, false, function(err, persistedSeries) {
            ...
            var update = {
                'displayName': seriesToImport.displayName,
                'description': seriesToImport.description
            };
            SeriesDAO.updateSerie(persistedSeries, update, callback);
        });
    } else {
        // Create the series
        return SeriesDAO.createSeries(seriesToImport.AppId, seriesToImport.displayName, seriesToImport.description, orgUnit.GroupId, seriesToImport.externalId, callback);
    }
};
 ...
 * @api private
 */
var _importSeriesEvents = function(events, series, callback) {
    events = events || [];
    if (_.isEmpty(events)) {
        return callback();
    }
    ...
    _importEvent(event, series, function(err) {
        if (err) {
            return callback(err);
        }

        _importSeriesEvents(events, series, callback);
    });
};
 ...
 * @api private
 */
var _importEvent = function(event, series, callback) {
    // Update or create the event record
    _updateOrCreateEvent(event, series, function(err, persistedEvent) {
        if (err) {
            return callback(err);
        }
        ...
        var organiserExternalIds = _.chain(event.organisers)
            ...
            .compact()
            .value();
        UsersDAO.getUsersByShibbolethId(series.AppId, organiserExternalIds, function(err, users) {
            if (err) {
                return callback(err);
            }

            // Map the user's their id to `true`
 ...
 * @api private
 */
var _updateOrCreateEvent = function(eventToImport, series, callback) {
    var opts = {
    ...
    if (eventToImport.id) {
        ...
        EventsDAO.getEvent(eventToImport.id, function(err, persistedEvent) {
            ...
            EventsDAO.updateEvent(persistedEvent, opts, callback);
        });
    } else {
        // Create the event
        ...
        EventsDAO.createEvent(series.AppId, opts.displayName, eventToImport.start, eventToImport.end, opts, callback);
    }
};
