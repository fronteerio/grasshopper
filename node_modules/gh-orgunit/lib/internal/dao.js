 ...
 */
var createOrgUnit = module.exports.createOrgUnit = function(appId, displayName, type, description, metadata, visibility, groupId, parentId, callback) {
    var orgUnit = {
        'AppId': appId,
...
var getOrgUnits = module.exports.getOrgUnits = function(appId, parentId, types, opts, callback) {
    ...
    if (opts.includeSeries) {
        var includeSeries = {'model': DB.Serie, 'required': false};

        // Indicate whether the user is subscribed to the serie
        if (opts.subscriptions.include && opts.subscriptions.userId) {
            includeSeries.include = [{'model': DB.Calendar, 'where': {'UserId': opts.subscriptions.userId}, 'required': false, 'limit': null}];
        }
