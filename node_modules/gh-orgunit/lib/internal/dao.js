...
var _exportOrgUnitCSV = function(rootId, orgUnits, callback) {
    ...
    _exportOrgUnitJSON(rootId, orgUnits, function(err, exportedOrgUnit) {
        ...
        var columns = _getExportCSVColumns(exportedOrgUnit);
...
/**
 * Get the column headers for a CSV export of an organisational unit tree. The column headers for
 * the organisational units contain the type of and the level on which the organisational unit appears.
 *
 * For example, a correct tree:
 * ```
 * + course
 *     + part
 *         + module
 ...
 * ```
 * course_0_id, course_0_externalId, .., part_1_id, part_1_externalId, .., module_2_id, module_2_externalId, ..
 * ```
 *
 * A tree which has the same type on different levels:
 * ```
 * + course
 *     + course
 *         + module
 ...
 * would get exported as:
 * ```
 * course_0_id, course_0_externalId, .., course_1_id, course_1_externalId, .., part_1_id, part_1_externalId, .., module_2_id, module_2_externalId, ..
 * ```
 *
 * @param  {Object}         exportedOrgUnit     A tree as generated by `exportOrgUnitJSON`
 * @return {String[]}                           The column headers for a CSV export
 * @api private
 */
var _getExportCSVColumns = function(exportedOrgUnit) {
    // Walk the tree and collect the levels on which each type appears. In a traditional
    // Timetable model each type should only appear on one level, but we will support types
    // appearing on multiple levels
    var types = {};
    var visitNodeForType = function(node, level) {
        var type = node.type;
        types[type] = types[type] || [];
        types[type].push(level);
    ...
    var columns = _.chain(types)
        ...
        .sortBy(function(item) {
            // Sort all the levels ascending
            item[1] = item[1].sort(function(a, b) { return a > b; });

            // Return the lowest level
            return item[1][0];
        })
        // Return the columns for each organisational unit type and the level it appears on
        .map(function(item) {
            var arr = _.map(item[1], function(level) {
                return [
                    util.format('%s_%d_id', item[0], level),
                    util.format('%s_%d_externalId', item[0], level),
                    util.format('%s_%d_displayName', item[0], level),
                    util.format('%s_%d_description', item[0], level),
                    util.format('%s_%d_published', item[0], level),
                    util.format('%s_%d_parentId', item[0], level)
                ];
            });
...
var _getExportCSVRecords = function(exportedOrgUnit) {
    ...
    var visitOrgUnit = function(orgUnit, record, level) {
        ...
        record[util.format('%s_%d_id', type, level)] = orgUnit.id;
        record[util.format('%s_%d_externalId', type, level)] = orgUnit.externalId;
        record[util.format('%s_%d_displayName', type, level)] = orgUnit.displayName;
        record[util.format('%s_%d_description', type, level)] = orgUnit.description;
        record[util.format('%s_%d_type', type, level)] = orgUnit.type;
        record[util.format('%s_%d_published', type, level)] = orgUnit.published;
        record[util.format('%s_%d_parentId', type, level)] = orgUnit.ParentId;
