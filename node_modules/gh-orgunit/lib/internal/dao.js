...
/**
 * Get the column headers for a CSV export of an organisational unit tree. If the tree contained
 * the same type of organisational unit on different levels, this function will return `null`.
 * This indicates that the specific tree is malformed and cannot be exported to CSV.
 *
 ...
 * ```
 * course_id, course_externalId, .., part_id, part_externalId, .., module_id, module_externalId, ..
 * ```
 *
 * An incorrect tree, which has the same type on different levels:
 * ```
 * + course
 *     + course
 *         + module
 ...
 * ```
 *
 * Now we can no longer use the `course` type to indicate a level in the tree as it might be the
 * root level or the next one. However, the next one conflicts with the part. This is why we do
 * not support having types appear on different levels
 *
...
var _getExportCSVColumns = function(exportedOrgUnit) {
    ...
    var visitNodeForType = function(node, level) {
        ...
        if (types[type] && types[type] !== level) {
            log().warn({'orgUnit': node.id}, 'Tried to export an organisational unit that has children with the same type but on different levels');
            malformed = true;
        }
...
var _getExportCSVRecords = function(exportedOrgUnit) {
    ...
    var visitOrgUnit = function(orgUnit, record) {
        ...
        record[orgUnit.type + '_type'] = orgUnit.type;
        record[orgUnit.type + '_published'] = orgUnit.published;
        record[orgUnit.type + '_ParentId'] = orgUnit.ParentId;
