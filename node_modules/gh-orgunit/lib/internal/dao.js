...
var deleteOrgUnitSeries = module.exports.deleteOrgUnitSeries = function(orgUnit, series, callback) {
    // Remove the series from the user calendars
    DB.CalendarSeries.destroy({
        'where': {
            'OrgUnitId': orgUnit.id,
            'SerieId': _.pluck(series, 'id')
        }
        ...
        if (err) {
            log().error({
                ...
                'id': orgUnit.id,
                'series': _.pluck(series, 'id')
            }, 'Unable to remove series from users their calendars when removing series from an organisational unit');
            return callback(err);
        }

        // Remove the actual series from under the organisational unit
