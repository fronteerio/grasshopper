...
var canLockGroup = module.exports.canLockGroup = function(ctx, group, callback) {
    ...
    canGetGroupMembers(ctx, group, function(err, canGetMembers) {
        ...
        // We also need to check the lock itself
        var isLockedForUser = (group.isLocked() && group.LockedBy && (group.LockedBy.id !== ctx.user.id && !ctx.user.canAdmin(group.AppId)));
        return callback(null, !isLockedForUser);
    });
};
...
var canReleaseLock = module.exports.canReleaseLock = function(ctx, group, callback) {
    // Ensure the current user can release a lock on the group. If you can
    // retrieve the members of a group, you are also allowed to release the lock on it
    canGetGroupMembers(ctx, group, function(err, canGetMembers) {
        if (err || !canGetMembers) {
            return callback(err, canGetMembers);
        }
