 ...
 * @QueryParam  {number}            [limit]                 The maximum number of results to retrieve. Default: 25
 * @QueryParam  {number}            [offset]                The paging number of the results to retrieve
 * @Return      {EventList}                                 The upcoming events for the user
 */
var _getUserMemberships = function(req, res) {
    GroupsAPI.getUserMemberships(req.ctx, req.params.id, req.query.limit, req.query.offset, function(err, groups) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }

        return res.status(200).send({'results': groups});
    });
};

GrassHopper.globalAdminRouter.on('get', '/api/users/:id/memberships', _getUserMemberships);
GrassHopper.appRouter.on('get', '/api/users/:id/memberships', _getUserMemberships);
 ...
 */
var _getGroupMembers = function(req, res) {
...
GrassHopper.globalAdminRouter.on('get', '/api/groups/:id/members', _getGroupMembers);
GrassHopper.appRouter.on('get', '/api/groups/:id/members', _getGroupMembers);
 ...
 * @Return      {void}
 */
var _updateGroupMembers = function(req, res) {
...
GrassHopper.globalAdminRouter.on('post', '/api/groups/:id/members', _updateGroupMembers);
GrassHopper.appRouter.on('post', '/api/groups/:id/members', _updateGroupMembers);
...
GrassHopper.appRouter.on('post', '/api/groups/:id/lock', function(req, res) {
    GroupsAPI.lock(req.ctx, req.params.id, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
GrassHopper.appRouter.on('delete', '/api/groups/:id/lock', function(req, res) {
    GroupsAPI.releaseLock(req.ctx, req.params.id, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
