 ...
 * @param  {Number}         [limit]             The amount of members that should be returned, defaults to 10
 ...
 *
 * @param  {Context}        ctx                 Standard context containing the current user and the current app
 * @param  {Number}         userId              The id of the user to get the memberships for
 * @param  {Number}         [limit]             The amount of groups that should be returned, defaults to 25
 * @param  {Number}         [offset]            The paging number of the results to retrieve
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error object, if any
 * @param  {Group[]}        callback.groups     The groups that the user is a member of
 */
var getUserMemberships = module.exports.getUserMemberships = function(ctx, userId, limit, offset, callback) {
    // Ensure that the paging values are valid
    limit = GrasshopperUtil.getNumberParam(limit, 25, 1, 1000);
    offset = GrasshopperUtil.getNumberParam(offset, 0, 0);

    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'Only authenticated users can list a user\'s memberships'}).isLoggedInUser(ctx);
    ...
    UsersDAO.getUser(userId, function(err, user) {
        ...
        // Ensure the user can get a user's members
        GroupsAuthz.canGetUserMemberships(ctx, user, function(err, canGetUserMemberships) {
            if (err) {
                return callback(err);
            } else if (!canGetUserMemberships) {
                log().warn({'userId': userId, 'actor': ctx.user.id}, 'Unauthorized attempt at retrieving a user\'s memberships');
                return callback({'code': 401, 'msg': 'You are not allowed to view this group\'s members'});
            }

            GroupsDAO.getUserMemberships(user, limit, offset, callback);
        });
    });
};
...
var updateGroupMembers = module.exports.updateGroupMembers = function(ctx, groupId, memberUpdates, callback) {
    ...
    _getGroup(ctx, ctx.app.id, groupId, function(err, group) {
        ...
        GroupsAuthz.canUpdateGroupMembers(ctx, group, memberUpdates, function(err, canUpdateGroupMembers) {
            ...
            // Ensure that the specified users exists and are on the same application
            var userIds = _.keys(memberUpdates);
            UsersDAO.getUsersById(group.AppId, userIds, function(err, users) {
                if (err) {
                    return callback(err);
                }
...
var getOrCreateGroup = module.exports.getOrCreateGroup = function(ctx, appId, groupId, callback) {
    ...
    if (groupId) {
        ...
        // Create the group and add the current user as a member if he's not a global admin
        var members = [];
        if (!ctx.user.isGlobalAdmin()) {
            members.push(ctx.user.id);
        }
        GroupsDAO.createGroup(appId, members, callback);
    }
};
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var lock = module.exports.lock = function(ctx, groupId, callback) {
    // Ensure that the group id is a valid number
    groupId = GrasshopperUtil.getNumberParam(groupId);

    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'Only authenticated users can lock a group'}).isLoggedInUser(ctx);
    validator.check(groupId, {'code': 400, 'msg': 'A valid group id must be provided'}).isInt();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    _getGroup(ctx, ctx.app.id, groupId, function(err, group) {
        ...
        // Ensure the current user can lock this group
        GroupsAuthz.canLockGroup(ctx, group, function(err, canLockGroup) {
            if (err) {
                return callback(err);
            } else if (!canLockGroup) {
                log().warn({'groupId': groupId, 'actor': ctx.user.id}, 'Unauthorized attempt at locking a group');
                return callback({'code': 401, 'msg': 'You are not allowed to lock this group'});
            }

            GroupsDAO.lock(group, ctx.user, callback);
        });
    });
};
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var releaseLock = module.exports.releaseLock = function(ctx, groupId, callback) {
    // Ensure that the group id is a valid number
    groupId = GrasshopperUtil.getNumberParam(groupId);

    var validator = new Validator();
    validator.check(null, {'code': 401, 'msg': 'Only authenticated users can release a lock on a group'}).isLoggedInUser(ctx);
    validator.check(groupId, {'code': 400, 'msg': 'A valid group id must be provided'}).isInt();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    _getGroup(ctx, ctx.app.id, groupId, function(err, group) {
        ...
        // Ensure the current user can release the lock on this group
        GroupsAuthz.canReleaseLock(ctx, group, function(err, canReleaseLock) {
            if (err) {
                return callback(err);
            } else if (!canReleaseLock) {
                log().warn({'groupId': groupId, 'actor': ctx.user.id}, 'Unauthorized attempt at releasing a lock on a group');
                return callback({'code': 401, 'msg': 'You are not allowed to release the lock on this group'});
            }

            GroupsDAO.releaseLock(group, callback);
        });
    });
};
