...
describe('Groups', function() {
    ...
    describe('Group creation', function() {
        ...
        it('verify a group has no members when created by a global admin', function(callback) {
            ...
            TestsUtil.getGlobalAdminRestClient(function(globalAdminClient) {
                ...
                OrgUnitTestsUtil.assertCreateOrgUnit(globalAdminClient, 'Test org unit', 'course', opts, function(orgUnit) {
    ...
    describe('Updating group members', function() {
        ...
        /**
         * Test that verifies that adding a user to a group is idempotent
         */
        it('verify adding a user to a group is idempotent', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2013, 1, false, function(nico) {

                // Create a new group by creating a new organisational unit
                OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'Test org unit', 'course', null, function(orgUnit) {

                    // Add Nico to the group
                    var update = {};
                    update[nico.profile.id] = true;
                    GroupsTestsUtil.assertUpdateGroupMembers(global.tests.admins.cam2013.client, orgUnit.GroupId, update, function() {

                        // Add Nico to the group again, this should have no effect
                        GroupsTestsUtil.assertUpdateGroupMembers(global.tests.admins.cam2013.client, orgUnit.GroupId, update, function() {
                            return callback();
                        });
                    });
    ...
    describe('Getting user memberships', function() {
        ...
        it('verify getting user memberships paging', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2013, 1, false, function(user) {
                ...
                OrgUnitTestsUtil.generateOrgUnits(global.tests.admins.cam2013.client, 30, function() {
                    ...
                    _addUserToGroups(global.tests.admins.cam2013.client, user.profile.id, groupIds, function() {
                        ...
                        // Sanity check the user is a member of all the groups
                        GroupsTestsUtil.assertGetAllUserMemberships(global.tests.admins.cam2013.client, user.profile.id, function(allMemberships) {
                            assert.strictEqual(allMemberships.length, 30);

                            // Verify that specifying no limit defaults to 25
                            GroupsTestsUtil.assertGetUserMemberships(global.tests.admins.cam2013.client, user.profile.id, null, null, function(memberships) {
                                assert.strictEqual(memberships.length, 25);

                                // Verify proper paging
                                // Get the first set of memberships
                                GroupsTestsUtil.assertGetUserMemberships(global.tests.admins.cam2013.client, user.profile.id, 15, null, function(firstPage) {
                                    assert.strictEqual(firstPage.length, 15);

                                    // Verify the next set of memberships
                                    GroupsTestsUtil.assertGetUserMemberships(global.tests.admins.cam2013.client, user.profile.id, 15, 15, function(secondPage) {
                                        assert.strictEqual(secondPage.length, 15);

                                        // Assert each membership does not appear in the first page
                                        _.each(secondPage, function(membership) {
                                            assert.ok(!_.find(firstPage, {'id': membership.id}));
                                        });
