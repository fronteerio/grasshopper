 ...
 * @param  {String}     [options.authenticationType]    Which authentication method should be used to authenticate with the APIs. If left blank, all requests will be made anonymously. Options are `local`
...
var createClient = module.exports.createClient = function(options, callback) {
    ...
    if (options.authenticationType === 'local') {
        client.authentication.login(options.username, options.password, function(err) {
            if (err) {
                return callback(err);
            }
...
var _createAnonymousClient = function(options) {
    ...
    require('./rest/authentication')(client);
