 ...
 *
 * @param  {App}            app             The application for which to generate an iCalendar feed
 ...
 */
var eventsToICal = module.exports.eventsToICal = function(app, calendarInfo, events) {
    // Construct a calendar
    // TODO: app.tenant
    var calendar = new VCalendar({
        'organization': app.displayName,
 ...
 *
 * @param  {App}            app             The application for which to generate an RSS feed
 * @param  {CalendarInfo}   calendarInfo    More information about the calendar
 * @param  {Event[]}        events          The events that should be in the RSS feed
 * @return {String}                         The RSS feed
 */
var eventsToRSS = module.exports.eventsToRSS = function(app, calendarInfo, events) {
    var header = [
    ...
    var calendar = [
        ...
        '<pubDate>', calendarInfo.lastModified, '</pubDate>',
    ...
    events = _.map(events, function(event) {
        var eventRSS = [
            '<item>',
                ...
                '<pubDate>', event.lastModified, '</pubDate>',
                '<link>', _createRSSLink(app, event.id), '</link>',
                '<guid>', _createRSSLink(app, event.id), '</guid>',
                '<ev:startdate>', event.start.toISOString(), '</ev:startdate>',
                '<ev:enddate>', event.end.toISOString(), '</ev:enddate>',
...
var _createRSSLink = function(app, eventId) {
    return 'https://' + app.host + '/events/' + eventId;
};
