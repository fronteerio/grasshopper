 ...
 */
var getServiceProviderUrl = module.exports.getServiceProviderUrl = function(ctx) {
    // The URL at which the Shibboleth SP software (`Apache` + `mod_shib`) is running
    var spURL = util.format('https://%s', getSPHost());

    // Generate a signature
    var data = {'app': ctx.app.id};
    var signature = Signature.createExpiringSignature(data, 60, 60);

    // Create and return the full URL
    return util.format('%s/api/auth/shibboleth/sp?app=%s&signature=%s&expires=%s', spURL, ctx.app.id, signature.signature, signature.expires);
};
 ...
 */
var getAuthenticatedUserRedirectUrl = module.exports.getAuthenticatedUserRedirectUrl = function(tenant, user) {
    var data = {'user': user.id};
    var signature = Signature.createExpiringSignature(data, 60, 60);
    return util.format('https://%s/api/auth/shibboleth/callback?user=%s&signature=%s&expires=%s', tenant.host, user.id, signature.signature, signature.expires);
};
