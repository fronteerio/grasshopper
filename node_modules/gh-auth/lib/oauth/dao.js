...
/**
 * Initialize the indexes and constraints of the OAuth data model
 *
...
var init = module.exports.init = function(callback) {
    ...
    // We join and look up OauthClients on AppId
    sequelize.query('CREATE INDEX oauth_clients_appid_idx ON "OauthClients" ("AppId");').complete(function(err) {
        if (err && err.message.indexOf('already exists') === -1) {
            return callback({'code': 500, 'msg': err.message});
        }

        // We look up OauthAuthCodes uniquely by their 'code' value
        sequelize.query('CREATE UNIQUE INDEX oauth_auth_codes_code_idx ON "OauthAuthCodes" ("code");').complete(function(err) {
            if (err && err.message.indexOf('already exists') === -1) {
                return callback({'code': 500, 'msg': err.message});
            }

            // We join and look up OauthAccessTokens on UserId, as well as UserId+OauthClientId
            sequelize.query('CREATE UNIQUE INDEX oauth_access_tokens_userclient_idx ON "OauthAccessTokens" ("UserId", "OauthClientId");').complete(function(err) {
                if (err && err.message.indexOf('already exists') === -1) {
                    return callback({'code': 500, 'msg': err.message});
                }

                // We join OauthAccessTokens on OauthClientId
                sequelize.query('CREATE INDEX oauth_access_tokens_client_idx ON "OauthAccessTokens" ("OauthClientId");').complete(function(err) {
                    if (err && err.message.indexOf('already exists') === -1) {
                        return callback({'code': 500, 'msg': err.message});
                    }

                    // We look up OauthAccessTokens uniquely by their token value
                    sequelize.query('CREATE UNIQUE INDEX oauth_access_tokens_token_idx ON "OauthAccessTokens" ("token");').complete(function(err) {
                        if (err && err.message.indexOf('already exists') === -1) {
                            return callback({'code': 500, 'msg': err.message});
                        }
 ...
 * @param  {String}         secret              The new client secret to set
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 ...
 * @param  {OauthClient}        callback.accessToken.OauthClient    The client associated to the access token
 ...
 * @param  {OauthAccessToken}   callback.accessToken    The access token associated to the given token
 */
var getAccessTokenByUserAndClient = module.exports.getAccessTokenByUserAndClient = function(user, client, callback) {
    var query = {
        'where': {
            'UserId': user.id,
            'OauthClientId': client.id
    ...
    DB.OauthAccessToken.find(query).complete(function(err, accessToken) {
        if (err) {
            log().error({
                ...
                'userId': user.id,
                'clientId': client.id
            }, 'Failed to fetch AccessToken by User and Client');
            return callback({'code': 500, 'msg': err.message});
        } else if (!accessToken) {
            log().debug({'userId': user.id, 'clientId': client.id}, 'Attempted to fetch non-existing access token');
 ...
 * @param  {Object}             callback.err    An error that occurred, if any
 */
var deleteAccessToken = module.exports.deleteAccessToken = function(accessToken, callback) {
