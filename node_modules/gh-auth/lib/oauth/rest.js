...
/**
 * Perform the OAuth 2.0 authorization code grant operation
 *
 * @param  {Request}    req     The request object performing the grant
 * @param  {Response}   res     The response object on which to send the response
 * @api private
 */
var _grant = function(req, res) {
    ...
    // Note that the grant flow also has a redirect_uri, but we don't acknowledge it here. Since we
    // currently don't support multiple redirect URIs, it's not necessary and instead always
    // redirect to the configured redirect URI for simplicity
 ...
 */
GrassHopper.appRouter.on('post', '/api/auth/oauth/grant', _grant);
GrassHopper.globalAdminRouter.on('post', '/api/auth/oauth/grant', _grant);
...
var _deleteAccessTokenByToken = function(req, res) {
    AuthOauth.deleteAccessTokenByToken(req.ctx, req.body.token, GrassHopperUtil.createResponseCallback(res));
};
...
GrassHopper.globalAdminRouter.on('delete', '/api/auth/oauth/accessToken/token', _deleteAccessTokenByToken);
...
GrassHopper.appRouter.on('delete', '/api/auth/oauth/accessToken/token', _deleteAccessTokenByToken);
