...
var _handleOauthClientAuthentication = function(req, clientId, clientSecret, done) {
    OauthDAO.getClient(clientId, function(err, client) {
        if (err) {
            ...
            return done({'code': 401, 'msg': 'Client secret mismatch'});
        } else if (req.ghApp.id !== client.AppId) {
            return done({'code': 401, 'msg': 'The provided client is not available on this app'});
        } else if (client.disabled) {
            return done({'code': 401, 'msg': 'This client has been disabled'});
        }
