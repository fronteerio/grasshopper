...
passport.use(new LocalStrategy({'passReqToCallback': true}, function(req, username, password, done) {
    DB.User.find({'where': {'username': username, 'TenantId': req.ctx.app.Tenant.id }}).complete(function (err, user) {
        if (err) {
 ...
 */
var localLoginSuccessCallback = function(req, res) {
    res.status(200).send(res.user);
};
GrassHopper.globalAdminServer.on('post', '/api/auth/login', passport.authenticate('local'), localLoginSuccessCallback);
GrassHopper.appServer.on('post', '/api/auth/login', passport.authenticate('local'), localLoginSuccessCallback);
