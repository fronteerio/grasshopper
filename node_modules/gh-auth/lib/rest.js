...
var _localLogin = function(req, res, next) {
    passport.authenticate(AuthConstants.strategies.LOCAL, function(err, authInfo) {
        if (err) {
            return next(err);
...
GrassHopper.globalAdminRouter.on('post', '/api/users/:id/password', _changePassword);
GrassHopper.appRouter.on('post', '/api/users/:id/password', _changePassword);
...
GrassHopper.appRouter.on('post', '/api/auth/shibboleth', function(req, res) {
    ...
    var redirectUrl = AuthUtil.validateRedirectUrl(req.body.redirectUrl);
...
GrassHopper.appRouter.on('get', '/api/auth/shibboleth/callback', function(req, res) {
    ...
    ShibbolethAPI.getUser(req.ghApp, user, signature, expires, function(err, user) {
        ...
        req.logIn(authInfo, function(err) {
            ...
            var redirectUrl = AuthUtil.validateRedirectUrl(req.signedCookies.redirectUrl);
