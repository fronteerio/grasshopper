...
var CalendarDAO = require('gh-calendar/lib/internal/dao');
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var subscribeSeries = module.exports.subscribeSeries = function(ctx, id, callback) {
    // Ensure that the serie id is a valid number
    id = GrasshopperUtil.getNumberParam(id);
    ...
    getSerie(ctx, id, function(err, serie) {
        if (err) {
            return callback(err);
        } else if (serie.AppId !== ctx.app.id) {
            return callback({'code': 401, 'msg': 'You cannot subscribe to a serie from another application'});
        }

        CalendarDAO.subscribeSeries(serie, ctx.user, callback);
    });
};
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var unsubscribeSeries = module.exports.unsubscribeSeries = function(ctx, id, callback) {
    // Ensure that the serie id is a valid number
    id = GrasshopperUtil.getNumberParam(id);
    ...
    getSerie(ctx, id, function(err, serie) {
        ...
        CalendarDAO.unsubscribeSeries(serie, ctx.user, callback);
    });
};
