...
var createSerie = module.exports.createSerie = function(ctx, appId, displayName, description, groupId, callback) {
    ...
    SeriesAuthz.canCreateSeries(ctx, function(err, canCreate) {
        ...
        GroupsAPI.getOrCreateGroup(ctx, appId, groupId, function(err, group) {
            ...
            SeriesDAO.createSeries(appId, displayName, description, group.id, null, null, callback);
        });
    });
};
...
var updateSerie = module.exports.updateSerie = function(ctx, id, update, callback) {
    ...
    getSerie(ctx, id, false, function(err, serie) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getSerie = module.exports.getSerie = function(ctx, id, includeOrgUnits, callback) {
    // Ensure that the series id is a valid number
    id = GrasshopperUtil.getNumberParam(id);

    var validator = new Validator();
    validator.check(id, {'code': 400, 'msg': 'A valid series id must be provided'}).isInt();
    if (includeOrgUnits) {
        // Ensure that the includeOrgUnits flag is a valid boolean
        includeOrgUnits = GrasshopperUtil.getBooleanParam(includeOrgUnits);
        validator.check(null, {'code': 400, 'msg': 'A valid include organisational units flag must be a boolean value'}).isBoolean(includeOrgUnits);
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    SeriesDAO.getSerie(id, includeOrgUnits, function(err, series) {
        if (err) {
            return callback(err);
        }

        // Ensure that the series is either being requested from the app it belongs to
        // or from an authenticated user on the global admin
        if (series.AppId !== ctx.app.id && (!ctx.user || !ctx.user.isGlobalAdmin())) {
            return callback({'code': 401, 'msg': 'This series cannot be retrieved on this application'});
        }

        // Check whether the current user can manage the series
        SeriesAuthz.canManageSeries(ctx, series, function(err, canManage) {
            if (err) {
                return callback(err);
            }

            series.canManage = canManage;
        ...
        SeriesAuthz.canAddSeriesEvents(ctx, serie, function(err, canAdd) {
            ...
            EventsDAO.getEvents(events, function(err, events) {
                ...
                SeriesDAO.getSeriesEvents(serie, null, null, 10000, 0, true, function(err, existingEvents) {
                    ...
                    // Get all the events that are NOT part of the series already
                    var newEvents = _.filter(events, function(event) {
                        return !_.find(existingEvents, function(existingEvent) {
 ...
 * @param  {Context}        ctx                 Standard context containing the current user and the current app
 * @param  {Number}         id                  The id of the serie to retrieve the events for
 * @param  {String}         start               The timestamp (ISO 8601) from which to get the event in the series
 * @param  {String}         end                 The timestamp (ISO 8601) until which to get the event in the series
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var subscribeSeries = module.exports.subscribeSeries = function(ctx, id, userId, orgUnit, callback) {
    ...
    if (orgUnit) {
        orgUnit = GrasshopperUtil.getNumberParam(orgUnit);
        validator.check(orgUnit, {'code': 400, 'msg': 'A valid organisational unit id must be provided'}).isInt();
    }
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    UsersDAO.getUser(userId, function(err, user) {
        ...
        getSerie(ctx, id, false, function(err, serie) {
            ...
            // Ensure the organisational unit exists, if any
            _validateOptionalOrgUnit(ctx, orgUnit, serie, function(err) {
                if (err) {
                    return callback(err);
                }
                ...
                SeriesAuthz.canSubscribeSerie(ctx, serie, user, function(err, canSubscribe) {
                    ...
                    UsersDAO.subscribeSeries([serie], user, orgUnit, callback);
                });
            });
        });
    });
 ...
 * @param  {Object}         callback.err        An error object, if any
 */
var _validateOptionalOrgUnit = function(ctx, orgUnitId, serie, callback) {
    if (!orgUnitId) {
        return callback();
    }
    ...
    OrgUnitDAO.getOrgUnit(orgUnitId, true, function(err, orgUnit) {
        ...
        // Ensure the organisational unit is on the same application as the serie
        } else if (serie.AppId !== orgUnit.AppId) {
            return callback({'code': 401, 'msg': 'The organisational unit must be on the same application as the serie'});
        }

        // Ensure the serie is under the organisational unit
        var orgUnitSerie = _.find(orgUnit.Series, function(orgUnitSerie) {
