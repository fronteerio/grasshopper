 ...
 * @param  {Number}         userId              The id of the user that should be subscribed
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error object, if any
 */
var subscribeSeries = module.exports.subscribeSeries = function(ctx, id, userId, callback) {
    // Ensure that the serie and user ids are valid numbers
    id = GrasshopperUtil.getNumberParam(id);
    ...
    UsersAPI.getUser(userId, function(err, user) {
        ...
        getSerie(ctx, id, function(err, serie) {
            ...
            SeriesAuthz.canSubscribeSerie(ctx, serie, user, function(err, canSubscribe) {
                ...
                SeriesDAO.subscribeSeries(serie, user, callback);
            });
        });
    });
};
 ...
 * @param  {Number}         userId              The id of the user that should be unsubscribed
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error object, if any
 */
    ...
    UsersAPI.getUser(userId, function(err, user) {
        ...
        getSerie(ctx, id, function(err, serie) {
            ...
            SeriesAuthz.canUnsubscribeSerie(ctx, serie, user, function(err, canUnsubscribe) {
                ...
                SeriesDAO.unsubscribeSeries(serie, user, callback);
            });
        });
    });
};
