...
var createSerie = module.exports.createSerie = function(ctx, appId, displayName, description, groupId, callback) {
    ...
    validator.check(appId, {'code': 400, 'msg': 'The application must be a positive integer'}).isInt();
    if (description) {
    ...
    if (groupId) {
        validator.check(groupId, {'code': 400, 'msg': 'A group must be a positive integer'}).isInt();
    }
...
var updateSerie = module.exports.updateSerie = function(ctx, id, update, callback) {
    ...
    validator.check(updatedFields.length, {'code': 400, 'msg': 'At least one of displayName, description or group should be specified'}).min(1);
    ...
    if (update.group) {
        // Ensure that the group id (if any) is a valid number
        update.group = GrasshopperUtil.getNumberParam(update.group);
    ...
    getSerie(ctx, id, function(err, serie) {
        ...
        SeriesAuthz.canUpdateSerie(ctx, serie, function(err, canUpdate) {
            ...
            _validateOptionalGroup(ctx, update.group, serie.AppId, function(err, group) {
                if (err) {
                    return callback(err);
                }
...
var getSeriesEvents = module.exports.getSeriesEvents = function(ctx, id, limit, offset, includeUpcoming, callback) {
    ...
    includeUpcoming = GrasshopperUtil.getBooleanParam(includeUpcoming);
 ...
 * @return {Object}                 An object representing the calendar info for the serie
 * @api private
 */
