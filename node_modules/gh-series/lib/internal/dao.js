...
var getSerie = module.exports.getSerie = function(id, includeOrgUnits, callback) {
    ...
    if (includeOrgUnits) {
        options.include.push({
            ...
            'include': [
                {'model': DB.OrgUnit},
                {'model': DB.OrgUnit, 'as': 'fromOrgUnit', 'attributes': ['id', 'displayName', 'ParentId']}
            ]
        });
    }
    ...
    DB.Serie.find(options).complete(function(err, serie) {
        if (err) {
            log().error({'err': err}, 'Failed to retrieve a series');
            return callback({'code': 500, 'msg': err.message});
        } else if (!serie) {
            log().debug({'id': id}, 'A series with the specified id could not be found');
            return callback({'code': 404, 'msg': 'Could not find a series by that id'});
        }

        if (includeOrgUnits) {
            var orgUnits = _.chain(serie.OrgUnitSeries)
                // Sort the organisational units by when the series was added to them
                .sortBy(function(orgUnitSerie) {
                    return orgUnitSerie.createdAt.getTime();
                })
                // Return just the organisational unit (and provide the "from" organisational unit if there is one)
                .map(function(orgUnitSerie) {
                    var orgUnit = orgUnitSerie.OrgUnit.toJSON();
                    orgUnit.fromOrgUnit = orgUnitSerie.fromOrgUnit;
                    return orgUnit;
                });

            // Serialize the series so Sequelize can't mess with the sorting of the organisational units
            serie = serie.toJSON();
            serie.OrgUnits = orgUnits;
        }
