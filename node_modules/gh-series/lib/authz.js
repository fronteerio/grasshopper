...
/**
 * Determine whether or not the user in context can manage a series
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current application
 * @param  {Serie}      series                      The series that the current wants to manage
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Boolean}    callback.canUpdate          `true` if the user in context has the appropriate permission. `false` otherwise
 */
var canManageSeries = module.exports.canManageSeries = function(ctx, series, callback) {
    // Anonymous users can never manage a series
    if (!ctx.user) {
        return callback(null, false);

    // Application administrators (or global admins) can always manage a series
    } else if (ctx.user.canAdmin(series.AppId)) {
        return callback(null, true);
    }

    // At this point we need to check if the user is a member of the series' group
    GroupsDAO.isMember(series.Group, ctx.user, callback);
};

/**
 * Determine whether or not the user in context can update a series
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current application
 * @param  {Serie}      series                      The series that the current user wants to update
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Boolean}    callback.canUpdate          `true` if the user in context has the appropriate permission. `false` otherwise
 */
var canUpdateSerie = module.exports.canUpdateSerie = function(ctx, series, callback) {
    return canManageSeries(ctx, series, callback);
};

/**
 * Determine whether or not the user in context can delete a series
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current application
 * @param  {Serie}      series                      The series that the current user wants to delete
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Boolean}    callback.canDelete          `true` if the user in context has the appropriate permission. `false` otherwise
 */
var canDeleteSerie = module.exports.canDeleteSerie = function(ctx, series, callback) {
    return canManageSeries(ctx, series, callback);
};

/**
 * Determine whether or not the user in context can add events to a series
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current application
 ...
 */
var canAddSeriesEvents = module.exports.canAddSeriesEvents = function(ctx, series, callback) {
    return canManageSeries(ctx, series, callback);
};

/**
 * Determine whether or not the user in context can delete events from a series
 *
 * @param  {Context}    ctx                         Standard context object containing the current user and the current application
 * @param  {Serie}      series                      The series from which the current user wants to delete events
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error that occurred, if any
 * @param  {Boolean}    callback.canDelete          `true` if the user in context has the appropriate permission. `false` otherwise
 */
var canDeleteSeriesEvents = module.exports.canDeleteSeriesEvents = function(ctx, series, callback) {
    return canManageSeries(ctx, series, callback);
};
