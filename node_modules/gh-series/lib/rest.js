 ...
 * @QueryParam  {boolean}           [includeOrgUnits]       Whether the organisational units that this serie is a part of should be included in the response. Defaults to `false`
 * @Return      {Series}                                    The requested event series
 */
var getSeries = function(req, res) {
    SeriesAPI.getSerie(req.ctx, req.params.id, req.query.includeOrgUnits, function(err, serie) {
        if (err) {
 ...
 * @FormParam   {number}            [orgUnit]           The id of the organisational unit to which the serie belongs
 * @Return      {void}
 */
var subscribeSeries = function(req, res) {
    SeriesAPI.subscribeSeries(req.ctx, req.params.id, req.body.userId, req.body.orgUnit, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
