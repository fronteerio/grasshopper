        ...
        TestsUtil.parseIcalCalendar(calendar, function(calendar) {
            ...
            if (expectedEvents) {
                ...
                _.each(calendar.subComponents, function(subComponent, i) {
                    ...
                    assert.strictEqual(id, expectedEvents[i].id);
                    assert.strictEqual(subComponent.model.description, expectedEvents[i].displayName);
                });
            }
 ...
 *
 * @param  {Object}             user                            The user who will be subscribing to a series
 * @param  {RestClient}         user.client                     The REST client to make the request with
 * @param  {User}               user.profile                    The profile of the user who will be subscringing to a series
 * @param  {Number}             id                              The id of the serie that the user should be subscribed to
 * @param  {Function}           callback                        Standard callback function
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertSubscribeSeries = module.exports.assertSubscribeSeries = function(subscriber, id, callback) {
    subscriber.client.serie.subscribeSeries(id, function(err) {
        assert.ok(!err);

        // Get the events in this serie so we can verify they appear in the user's calendar
        assertGetSeriesEvents(subscriber.client, id, null, null, null, null, function(events) {
            ...
            if (!_.isEmpty(events.results.length)) {
                start = moment(_.first(events.results).start).subtract(1, 'hours').format();
                end = moment(_.last(events.results).end).add(1, 'hours').format();
            }

            // Get the user's calendar
            UsersTestsUtil.assertGetUserCalendar(subscriber.client, subscriber.profile.id, start, end, events.results, function(calendar) {
                return callback();
            });
        });
    });
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertSubscribeSeriesFails = module.exports.assertSubscribeSeriesFails = function(client, id, code, callback) {
    client.serie.subscribeSeries(id, function(err) {
        assert.ok(err);
        assert.strictEqual(err.code, code);
        return callback();
    });
 ...
 *
 * @param  {Object}             user                            The user who will be unsubscribing from a series
 * @param  {RestClient}         user.client                     The REST client to make the request with
 * @param  {User}               user.profile                    The profile of the user who will be unsubscribing from a series
 * @param  {Number}             id                              The id of the serie that the user should be unsubscribed from
 * @param  {Function}           callback                        Standard callback function
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertUnsubscribeSeries = module.exports.assertUnsubscribeSeries = function(subscriber, id, callback) {
    subscriber.client.serie.unsubscribeSeries(id, function(err) {
        assert.ok(!err);

        // Get the events in this serie so we can verify they no longer appear in the user's calendar
        assertGetSeriesEvents(subscriber.client, id, null, null, null, null, function(serieEvents) {
            ...
            if (!_.isEmpty(serieEvents.results.length)) {
                start = moment(_.first(serieEvents.results).start).subtract(1, 'hours').format();
                end = moment(_.last(serieEvents.results).end).add(1, 'hours').format();
            }

            // Get the user's calendar
            UsersTestsUtil.assertGetUserCalendar(subscriber.client, subscriber.profile.id, start, end, null, function(calendar) {
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertUnsubscribeSeriesFails = module.exports.assertUnsubscribeSeriesFails = function(client, id, code, callback) {
    client.serie.unsubscribeSeries(id, function(err) {
        assert.ok(err);
        assert.strictEqual(err.code, code);
        return callback();
    });
