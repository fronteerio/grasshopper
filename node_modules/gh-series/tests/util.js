...
var Listener = require('gh-series/lib/internal/listener');
...
var assertGetSeriesEvents = module.exports.assertGetSeriesEvents = function(client, id, limit, offset, upcoming, expectedEvents, callback) {
    client.serie.getSeriesEvents(id, limit, offset, upcoming, function(err, events) {
        ...
        var eventStarts = _.pluck(events.results, 'start');
        assert.deepEqual(eventStarts, eventStarts.slice().sort());
    ...
    client.serie.subscribeSeries(id, userId, orgUnit, function(err) {
        ...
        UsersTestsUtil.assertGetMe(client, function(me) {
            ...
            assertGetSeriesEvents(client, id, null, null, null, null, function(events) {
                ...
                var startEndRange = _findStartAndEndRange(events.results);
                if (startEndRange) {
                ...
                UsersTestsUtil.assertGetUserCalendar(client, userId, start.format(), end.format(), events.results, function(calendar) {
                    return callback();
                });
            });
        });
...
/**
 * Given a list of events, find the start -> end date range from the start of the earliest beginning
 * event to the end of the last finishing event
 *
 * @param  {Event[]}    events  The events whose start/end range to find
 * @return {String[]}           An array of 2 timestamps (ISO 8601). The first one is the start time, and last one is the end time, inclusive. When `null`, indicates that the array was empty
 */
var _findStartAndEndRange = function(events) {
    if (_.isEmpty(events)) {
        return null;
    }

    // If the series contained events, we generate a window around the series events
    // and assert that the events are returned in the user's calendar
    var startTimestamp = _.chain(events).sortBy('start').first().value().start;
    var endTimestamp = _.chain(events).sortBy('end').last().value().end;
