 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertGetSerie = module.exports.assertGetSerie = function(client, id, includeOrgUnits, expectedSerie, callback) {
    // Wait until all serie and/or event updates have been dealt with
    serieEventsCounter.whenZero(function() {

        // Get the serie
        client.serie.getSerie(id, includeOrgUnits, function(err, serie) {
            assert.ok(!err);
            assert.ok(serie);
            assert.strictEqual(serie.id, id);
            if (includeOrgUnits) {
                assert.ok(_.isArray(serie.OrgUnits));
            }
            if (expectedSerie) {
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertGetSerieFails = module.exports.assertGetSerieFails = function(client, id, includeOrgUnits, code, callback) {
    client.serie.getSerie(id, includeOrgUnits, function(err, serie) {
        assert.ok(err);
        assert.strictEqual(err.code, code);
        assert.ok(!serie);
    ...
    assertCreateSerie(client, displayName, null, function(serie) {
        ...
        EventsTestsUtil.generateTestEvents(client, eventsPerSerie, rangeStart, rangeEnd, function(events) {
            ...
            assertAddSeriesEvents(client, serie.id, eventIds, function() {
                ...
                assertGetSerie(client, serie.id, false, null, function(serie) {
                    serie.events = events;
                    _series.push(serie);
