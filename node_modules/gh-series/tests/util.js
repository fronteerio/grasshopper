    ...
    client.serie.subscribeSeries(id, userId, orgUnit, function(err) {
        ...
        UsersTestsUtil.assertGetMe(client, function(me) {
            ...
            assertGetSeriesEvents(client, id, null, null, null, null, function(events) {
                ...
                if (!_.isEmpty(events.results)) {
                    var startDateTime = _.chain(events.results).sortBy('start').first().value().start;
                    var endDateTime = _.chain(events.results).sortBy('end').last().value().end;
                    start = moment(startDateTime).subtract(1, 'hours').format();
                    end = moment(endDateTime).add(1, 'hours').format();
                }
