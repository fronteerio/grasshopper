...
var assertGetAppAdmins = module.exports.assertGetAppAdmins = function(client, id, limit, offset, expectedResults, expectedTotal, callback) {
    client.app.getAppAdmins(id, limit, offset, function(err, appAdmins) {
        assert.ok(!err);
        assert.ok(appAdmins);
        assert.ok(appAdmins.count !== undefined);
        assert.ok(appAdmins.rows !== undefined);

        if (expectedResults) {
            assert.equal(appAdmins.rows.length, expectedResults);
        }

        if (expectedTotal) {
            assert.equal(appAdmins.count, expectedTotal);
        }

        _.each(appAdmins.rows, function(appAdmin) {
            assert.equal(appAdmin.AppId, id);
        });
...
var assertUpdateApp = module.exports.assertUpdateApp = function(client, app, opts, callback) {
    client.app.updateApp(app.id, opts, function(err, updatedApp) {
        ...
        assert.equal(updatedApp.id, app.id);
        assert.equal(updatedApp.createdAt, app.createdAt);
        assert.notEqual(updatedApp.updatedAt, app.updatedAt);

        if (opts.displayName) {
            assert.equal(updatedApp.displayName, opts.displayName);
        } else {
            assert.equal(updatedApp.displayName, app.displayName);
        }

        if (opts.enabled !== undefined) {
            assert.equal(updatedApp.enabled, opts.enabled);
        } else {
            assert.equal(updatedApp.enabled, app.enabled);
        }

        if (opts.host) {
            assert.equal(updatedApp.host, opts.host);
        } else {
            assert.equal(updatedApp.host, app.host);
        }
