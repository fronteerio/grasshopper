...
var getAppAdmins = module.exports.getAppAdmins = function(id, limit, offset, callback) {
    DB.User.findAndCountAll({'where': {'isAdmin': true, 'AppId': id}, 'limit': limit, 'offset': offset}).complete(function(err, globalAdmins) {
        if (err) {
...
var updateAppAdmins = module.exports.updateAppAdmins = function(adminUpdates, callback) {
    var adminsUpdated = _.after(_.keys(adminUpdates).length, callback);

    _.each(adminUpdates, function(adminStatus, userId) {
        DB.User.update({'isAdmin': adminStatus}, {'where': {'id': userId}}).complete(adminsUpdated);
    });
};