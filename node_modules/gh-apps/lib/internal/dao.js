...
var getApp = module.exports.getApp = function(id, callback) {
    DB.App.find(id).complete(function(err, app) {
        if (err) {
            log().error({'err': err, 'id': id}, 'Failed to get an app');
            return callback({'code': 500, 'msg': err.message});
        } else if (!app) {
            log().error({'err': err}, 'An app with the specified id could not be found');
...
var getAppByHost = module.exports.getAppByHost = function(host, callback) {
    DB.App.find({'where': {'host': host}}).complete(function(err, app) {
        if (err) {
            log().error({'err': err, 'host': host}, 'Failed to get an app by host name');
            return callback({'code': 500, 'msg': err.message});
        } else if (!app) {
            log().error({'err': err}, 'An app with the specified host could not be found');
