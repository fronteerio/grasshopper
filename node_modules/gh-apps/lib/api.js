...
var AppsConstants = require('./constants');
...
var getApps = module.exports.getApps = function(ctx, tenantId, callback) {
    ...
    tenantId = GrasshopperUtil.getNumberParam(tenantId);
    ...
    TenantsAPI.getTenant(ctx, tenantId, function(err, tenant) {
        ...
        return AppsDAO.getApps(tenant.id, callback);
    });
};
...
var createApp = module.exports.createApp = function(ctx, tenantId, displayName, host, type, callback) {
    ...
    validator.check(displayName, {'code': 400, 'msg': 'An app name can be at most 255 characters long'}).isShortString();
    validator.check(host, {'code': 400, 'msg': 'An app host must be provided'}).notEmpty();
    validator.check(host, {'code': 400, 'msg': 'An app host can be at most 255 characters long'}).isShortString();
    validator.check(type, {'code': 400, 'msg': 'An app type must be provided'}).notEmpty();
    validator.check(type, {'code': 400, 'msg': 'An app type must be provided'}).isIn(_.values(AppsConstants.types));
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
...
var updateApp = module.exports.updateApp = function(ctx, id, opts, callback) {
    ...
    if (opts) {
        ...
        if (!_.isUndefined(opts.displayName)) {
            validator.check(opts.displayName, {'code': 400, 'msg': 'An app name can be at most 255 characters long'}).isShortString();
        }
        if (!_.isUndefined(opts.enabled)) {
            opts.enabled = GrasshopperUtil.getBooleanParam(opts.enabled);
            validator.check(null, {'code': 400, 'msg': 'A valid boolean must be provided when enabling/disabling an app'}).isBoolean(opts.enabled);
        }
        if (!_.isUndefined(opts.host)) {
            validator.check(opts.host, {'code': 400, 'msg': 'An app host can be at most 255 characters long'}).isShortString();
            // Lower case the host name
...
var updateAppAdmins = module.exports.updateAppAdmins = function(ctx, id, adminUpdates, callback) {
    ...
    _.each(adminUpdates, function(adminValue, userId) {
        adminUpdates[userId] = GrasshopperUtil.getBooleanParam(adminValue);
        validator.check(null, {'code': 400, 'msg': 'A valid boolean must be provided to update an app administrator'}).isBoolean(adminUpdates[userId]);
    ...
    getApp(ctx, id, function(err, app) {
        ...
        UsersDAO.getUsersById(id, userIds, function(err, users) {
            if (err) {
                return callback(err);
            }
