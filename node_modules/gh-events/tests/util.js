...
/**
 * Assert that the organisers for an event can be updated
 *
 * @param  {RestClient}         client                          The REST client to make the request with
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertUpdateEventOrganisers = module.exports.assertUpdateEventOrganisers = function(client, id, update, callback) {
    client.event.updateEventOrganisers(id, update, function(err, event) {
        assert.ok(!err);
        assert.ok(event);
        assert.strictEqual(event.id, id);

        // Get the event and assert the update was persisted correctly
        assertGetEvent(client, id, null, function(event) {

            // Index the organisers by their name or id
            var organisersByIdOrName = _.indexBy(event.organisers, function(organiser) {
                if (organiser.id) {
                    return organiser.id;
                } else {
                    return organiser;
            ...
            // Verify that each update we made was persisted
            _.each(update, function(val, user) {
                if (val) {
                    assert.ok(organisersByIdOrName[user]);
                } else {
                    assert.ok(!organisersByIdOrName[user]);
                }
            });
...
/**
 * Assert that the organisers for an event can not be updated
 *
 * @param  {RestClient}         client                          The REST client to make the request with
 * @param  {Number}             id                              The id of the event for which to update the organisers
 * @param  {Object}             update                          The updates to persist
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertUpdateEventOrganisersFails = module.exports.assertUpdateEventOrganisersFails = function(client, id, update, code, callback) {
    client.event.updateEventOrganisers(id, update, function(err, event) {
        assert.ok(err);
        assert.strictEqual(err.code, code);
        assert.ok(!event);
...
var generateTestEvents = module.exports.generateTestEvents = function(client, total, rangeStart, rangeEnd, callback, _events) {
    ...
    var startFormatted = startFuzzed.format();

    // The end date for the event is the start date + anything from 15 minutes to 7 hours
    var endFuzzed = startFuzzed.add(15, 'minute').add(_.random(25200), 'second');
