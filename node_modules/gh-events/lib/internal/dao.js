 ...
 * @param  {Event}      callback.event      The updated event
 */
var updateEventOrganisers = module.exports.updateEventOrganisers = function(event, update, callback) {
    // TODO: Transactions
    // Split out the update in organisers that are address by user id or provided as plain text
    var users = _.chain(update)
        .keys()
    ...
    getOrganisers(event, function(err, organisers) {
        ...
        _.each(users.byUserId, function(userId) {
            if (update[userId]) {
        ...
        event.setOrganisers(organiserIds).complete(function(err) {
            ...
            var plainTextOrganisersByName = {};
            _.each(event.organiserOther, function(name) {
                plainTextOrganisersByName[name] = true;
            });

            // Add and/or remove the plain-text organisers
            _.each(users.byPlainText, function(user) {
                if (update[user]) {
                    plainTextOrganisersByName[user] = true;
                } else {
                    delete plainTextOrganisersByName[user];
                }
            });

            // Persist the plain-text organisers
            var plainTextOrganisers = _.keys(plainTextOrganisersByName);
