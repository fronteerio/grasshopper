...
var updateEventOrganisers = function(req, res) {
    EventsAPI.updateEventOrganisers(req.ctx, req.params.id, req.body, function(err, event) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
GrassHopper.globalAdminRouter.on('post', '/api/events/:id/organisers', updateEventOrganisers);
GrassHopper.appRouter.on('post', '/api/events/:id/organisers', updateEventOrganisers);
