...
var createEvent = module.exports.createEvent = function(ctx, appId, displayName, start, end, opts, callback) {
    ...
    EventsAuthz.canCreateEvent(ctx, appId, function(err, canCreateEvent) {
        if (err) {
            return callback(err);
...
var updateEvent = module.exports.updateEvent = function(ctx, id, update, callback) {
    ...
    if (update.group) {
        validator.check(update.group, {'code': 400, 'msg': 'If provided, a group must be a valid integer'}).isInt();
    }
    if (update.location) {
