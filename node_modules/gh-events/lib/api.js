 ...
 * @param  {Context}        ctx                     Standard context containing the current user and the current app
 * @param  {Number}         id                      The id of the event to update
 * @param  {Object}         update                  The updates to persist. Keys that are numbers are considered to be references to users, anything else is considered to be a plain-text organiser
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error object, if any
 * @param  {Event}          callback.event          The updated event
 */
var updateEventOrganisers = module.exports.updateEventOrganisers = function(ctx, id, update, callback) {
    update = update || {};
    ...
    _.each(update, function(val, user) {
        update[user] = GrasshopperUtil.getBooleanParam(update[user]);
        validator.check(null, {'code': 400, 'msg': 'A valid organiser change must be a boolean value'}).isBoolean(update[user]);
    });
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    getEvent(ctx, id, function(err, event) {
        ...
        // Get the user ids, if any, from the update
        var userIds = _.chain(update)
            .keys()
            .map(function(user) {
                return parseInt(user, 10);
            })
            .filter(function(val) {
                // Note that we cannot use _.isNumber here as NaN is considered to be a Number too
        ...
        // Ensure the specified users exist, if any
        _validateOptionalUsers(ctx, userIds, event.AppId, function(err, users) {
            if (err) {
                return callback(err);
            }
            ...
            EventsAuthz.canUpdateEvent(ctx, event, function(err, canUpdateEvent) {
                ...
                // TODO: Add a "confirm"-flow for the organisers, so regular users can't add other
                // users willy-nilly to their own events?
                ...
                // further down the future
                var oldEvent = _.clone(event.toJSON());
                EventsDAO.updateEventOrganisers(event, update, function(err, updatedEvent) {
                    if (err) {
                        return callback(err);
                    }
...
var _validateOptionalUsers = function(ctx, userIds, appId, callback) {
    ...
    UsersDAO.getUsers(appId, userIds, callback);
};
