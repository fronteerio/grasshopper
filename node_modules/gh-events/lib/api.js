...
var createEvent = module.exports.createEvent = function(ctx, appId, displayName, start, end, opts, callback) {
    ...
    if (opts.location) {
        validator.check(opts.location, {'code': 400, 'msg': 'A location can be at most 255 characters long'}).isShortString();
    }
    if (opts.notes) {
    ...
    if (opts.organiserOther) {
        ...
        _.each(opts.organiserOther, function(organiser) {
            validator.check(organiser, {'code': 400, 'msg': 'A plain-text organiser can be at most 255 characters long'}).isShortString();
    ...
    _validateOptionalSeries(ctx, opts.series, appId, function(err, series) {
        ...
        EventsAuthz.canCreateEvent(ctx, appId, series, function(err, canCreateEvent) {
            ...
            UsersDAO.getUsersById(appId, userIds, function(err, users) {
                ...
                // If no organisers were provided, we default the event organiser to the current user.
                // We only do this when the application is not a timetable application
                if (_.isEmpty(opts.organiserOther) && _.isEmpty(opts.organiserUsers) &&
                    ctx.app.type !== AppsConstants.types.TIMETABLE && !ctx.user.isGlobalAdmin()) {
 ...
 * @param  {String}         [update.end]                Updated timestamp (ISO 8601) at which the event ends
...
var updateEvent = module.exports.updateEvent = function(ctx, id, update, callback) {
    ...
    if (update.location) {
        validator.check(update.location, {'code': 400, 'msg': 'A location can be at most 255 characters long'}).isShortString();
    }
    if (update.notes) {
...
var _validateOptionalSeries = function(ctx, series, appId, callback) {
    ...
    SeriesDAO.getSeries(series, function(err, series) {
        ...
        var otherAppSerie = _.find(series, function(serie) {
            return (serie.AppId !== appId);
        });
        if (otherAppSerie) {
