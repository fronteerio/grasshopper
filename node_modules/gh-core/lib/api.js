...
var init = module.exports.init = function(config, callback) {
    ...
    DB.init(config, function(err) {
        ...
        DB.Tenant.findOrCreate({'where': {'displayName': 'Global admin tenant'}, 'defaults': {'displayName': 'Global admin tenant'}}).complete(function(err, tenant, created) {
            if (err) {
            ...
            DB.App.findOrCreate({'where': {'hostname': config.servers.adminHostname}, 'defaults': appOpts}).complete(function(err, app, created) {
                if (err) {
