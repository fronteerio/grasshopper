...
var _setUpModel = function(sequelize) {
     ...
     * @property  {Boolean}     isGlobalAdmin           Whether the application represents the global administration application
    ...
    var User = module.exports.User = sequelize.define('User', {
        'authenticationStrategy': {
            'type': Sequelize.ENUM(_.values(AuthConstants.strategies)),
            'allowNull': false
        },
        'displayName': {
            'type': Sequelize.STRING,
        ...
        'email': {
            'type': Sequelize.STRING,
            'allowNull': true
            // Note: This field used to be required to be unique within an app. This is no longer the case
            // as email/password logins are no longer a requirement and it is possible for different shibboleth
        ...
        'instanceMethods': {
            ...
            'hidePersonalInformation': function(ctx) {
                ...
                // Anonymous users or users from other applications cannot see the Shibboleth id
                } else if (!ctx.user || ctx.user.AppId !== this.AppId) {
     ...
     *
     * @property  {String}      displayName     The name of the client
     * @property  {String}      redirectUri     The uri to which granted authorization codes are redirected
     * @property  {String}      secret          The client secret, used for client authentication
     * @property  {Date}        disabled        The date on which this client was disabled, if applicable
     * @property  {String}      AppId           The id of the app to which this client belongs
     */
    var OauthClient = module.exports.OauthClient = sequelize.define('OauthClient', {
        'displayName': {
            'type': Sequelize.TEXT,
            'allowNull': false
        },
    ...
    // Many clients can be created per App
    App.hasMany(OauthClient);
    OauthClient.belongsTo(App);

    /**
     * The OAuth Authentication Code model
     *
     * @property  {String}      code            The unique string code value that was granted
     * @property  {Date}        expires         When the code expires
     * @property  {String}      OauthClientId   The id of the client to which the code belongs
     * @property  {String}      UserId          The id of the user that granted the code for the client
     */
    var OauthAuthCode = module.exports.OauthAuthCode = sequelize.define('OauthAuthCode', {
        'code': {
            'type': Sequelize.TEXT,
            'allowNull': false
        },
    ...
    // A client can have many users grant auth codes for them
    OauthClient.hasMany(OauthAuthCode);
    OauthAuthCode.belongsTo(OauthClient);

    // A user can grant many auth tokens for a client
    User.hasMany(OauthAuthCode);
    OauthAuthCode.belongsTo(User);

    /**
     * The OAuth Access Token model
     *
     * @property  {String}      tokne           The unique string token value
     * @property  {String}      OauthClientId   The id of the client that exchanged for this token
     * @property  {String}      UserId          The id of the user to which this token belongs
     */
    var OauthAccessToken = module.exports.OauthAccessToken = sequelize.define('OauthAccessToken', {
        'token': {
            'type': Sequelize.TEXT,
            'allowNull': false
        },
    ...
    // A client can have many users authorize tokens for them
    OauthClient.hasMany(OauthAccessToken);
    OauthAccessToken.belongsTo(OauthClient);

    // A user can authorize many access tokens
    User.hasMany(OauthAccessToken);
    ...
    var Config = module.exports.Config = sequelize.define('Config', {
        ...
        'shibMapDisplayname': {
            'type': Sequelize.TEXT,
            'defaultValue': 'displayname cn eppn'
        },
        'shibMapEmail': {
            'type': Sequelize.TEXT,
