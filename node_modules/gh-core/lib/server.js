...
var setUpServer = module.exports.setUpServer = function(port, config) {
    ...
    // TODO: Move this to gh-authentication once we have authentication
    app.use(function(req, res, next) {
        if (req.hostname === config.servers.adminHostname) {
            ...
            req.ctx = new Context(adminApp);
            return next();
        }

        DB.App.find({'where': {'hostname': req.hostname}, 'include': [DB.Tenant]}).complete(function(err, app) {
            if (err) {
