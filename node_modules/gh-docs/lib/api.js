...
var log = require('gh-core/lib/logger').logger('gh-core/swagger');
...
var addModelsToResources = module.exports.addModelsToResources = function() {
    ...
    _.each(appResources, function(resource) {
        _addModelsToResource(resource);
    });
};

/**
 * Finds all the rest*.js files in a module's `lib` directory and runs them through the restjsdoc parser
 *
...
var documentModule = module.exports.documentModule = function(moduleName, callback) {
    ...
    readdirp({'root': util.format('%s/../../%s/lib', __dirname, moduleName), 'fileFilter': 'rest*.js'})
        ...
        try {
            ...
            _.each(doc.endpoints, function(endpoint) {
                ...
                _.each(endpoint.server.split(','), function(server) {
                    if (server === 'app') {
                        _addSwaggerEndpoint(endpoint, appResources);
                    } else if (server === 'admin') {
                        _addSwaggerEndpoint(endpoint, adminResources);
                    } else {
                        log().warn({'file': filePath}, 'Tried to register swagger docs for unknown server "' + endpoint.server + '"');
                    }
                });
            });
        } catch (ex) {
...
var getApi = module.exports.getApi = function(ctx, swaggerResourcePath) {
    var resources = _getSwaggerResources(ctx);
    var url = '/api';
    var api = _.cloneDeep(resources[swaggerResourcePath]);
    api.basePath = url;
    return api;
};
...
/**
 * Get all the swagger resources for the appropriate app
 *
 * @param  {Context}    ctx     Standard context object containing the current user and the current app
 * @return {Object}             An object containing all the swagger resource info associated to the app
 * @api private
 */
var _getSwaggerResources = function(ctx) {
    if (ctx.app.isGlobalAdmin) {
