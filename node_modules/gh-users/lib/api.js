 ...
 *
 * @param  {Context}        ctx                 Standard context object containing the current user and the current applicatio
 * @param  {Number}         [appId]             The application to search users in, defaults to the current application
 ...
 */
var getUsers = module.exports.getUsers = function(ctx, app, filter, limit, offset, callback) {
    // Default some parameters
    app = GrasshopperUtil.getNumberParam(app) || ctx.app.id;
    ...
    if (app !== ctx.app.id && (!ctx.user || !ctx.user.canAdmin(app))) {
    ...
    // If a filter was specified, search for users
    if (filter) {
        UsersDAO.searchUsers(app, filter, limit, offset, callback);
 ...
 *
 * @param  {Context}        ctx                             The current execution context containing the user and application
 * @param  {Number}         appId                           The id of the user that should be updated
