 ...
 * @param  {Object}         callback.err            An error that occurred, if any
 */
var changePassword = module.exports.changePassword = function(ctx, userId, newPassword, password, callback) {
    userId = GrasshopperUtil.getNumberParam(userId);
    ...
    validator.check(newPassword, {'code': 400, 'msg': 'A valid new password must be provided'}).len(6);
    validator.check(password, {'code': 400, 'msg': 'A valid password of the user making the change must be provided'}).notEmpty();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    // Ensure the user that is making the change specified their own password
    ...
    UsersDAO.getUser(userId, function(err, user) {
        ...
        // Ensure the user is using local authentication
        } else if (!user.password) {
            return callback({'code': 400, 'msg': 'This user is not using local authentication'});
        }

        // Ensure the current user can change the user's password
        UsersAuthz.canChangePassword(ctx, user, function(err, canChangePassword) {
            if (err) {
                return callback(err);
            } else if (!canChangePassword) {
                log().warn({'id': userId, 'actor': ctx.user.id}, 'Unauthorized attempt at changing a user\'s password');
                return callback({'code': 401, 'msg': 'You are not allowed to update this user\'s password'});
            }

            var update = {
                'password': AuthUtil.hashPassword(newPassword)
            };
            UsersDAO.updateUser(user, update, function(err) {
                if (err) {
                    return callback(err);
                }
                ...
                log().info({
                    'actor': ctx.user,
                    'user': user
                }, 'Updated the password of a user');
