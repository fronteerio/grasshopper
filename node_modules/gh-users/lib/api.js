    ...
    AppsAPI.getApp(ctx, appId, function(err, app) {
        ...
        UsersAuthz.canImportUsers(ctx, app, function(err, canImportUsers) {
            if (err) {
                ...
                log().warn({'app': appId, 'actor': ctx.user.id}, 'Unauthorized attempt at importing users');
                return callback({'code': 401, 'msg': 'You are not allowed to import users'});
            }

            // Construct the CSV parser
            var parser = csv.parse({'trim': true}, function(err, users) {
                ...
                _cleanUpCSVFile(file, function() {
                    ...
                    // CSV file's users
                    _.each(users, function(user) {
                        validator.check(user[0], {'code': 400, 'msg': 'Missing display name for: ' + user[1]}).notEmpty();
                        if (authenticationStrategy === 'local') {
                            validator.check(user[1], {'code': 400, 'msg': 'A valid email address must be provided for: ' + user[0]}).isEmail();
                            validator.check(user[2], {'code': 400, 'msg': 'A valid password must be provided for: ' + user[0]}).notEmpty();
                            validator.check(user[2], {'code': 400, 'msg': 'A valid password must be provided for: ' + user[0]}).len(6);
                        } else {
                            validator.check(user[2], {'code': 400, 'msg': 'A valid shibboleth id must be provided for: ' + user[0]}).notEmpty();
                        }
                    });
                    ...
                    _importUsers(appId, authenticationStrategy, forceProfileUpdate, users, function(err) {
                        if (!err) {
                            log().info({
                                'app': appId,
                                'authenticationStrategy': authenticationStrategy
            ...
            var input = fs.createReadStream(file.file);
            input.pipe(parser);
        });
    });
};
 ...
 *
 * @param  {Number}     app                         The id of the application to import the users into
 * @param  {String}     authenticationStrategy      The authentication strategy for the user. One of `local` or `shibboleth`
 ...
 * @param  {Object[]}   users                       The users to import
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error object, if any
 * @api private
 */
var _importUsers = function(app, authenticationStrategy, forceProfileUpdate, users, callback) {
    if (_.isEmpty(users)) {
        return callback();

    // Add a progress log statement every 50 imported users
    } else if (users.length % 50 === 0) {
        log().info({
            'authenticationStrategy': authenticationStrategy,
            'app': app
        }, 'Importing users from CSV. ' + users.length + ' users left to import');
    }

    // Import the next user
    var user = users.pop();
    _importUser(app, authenticationStrategy, forceProfileUpdate, user, function(err) {
        ...
        _importUsers(app, authenticationStrategy, forceProfileUpdate, users, callback);
    });
};
 ...
 * Import a user
 *
 * @param  {Number}     app                         The id of the application to import the user into
 * @param  {String}     authenticationStrategy      The authentication strategy for the user. One of `local` or `shibboleth`
 * @param  {Boolean}    [forceProfileUpdate]        Whether the user information should be updated, even when other user information is already present
 * @param  {Object}     user                        The user to import
 * @param  {Function}   callback                    Standard callback function
 * @param  {Object}     callback.err                An error object, if any
 * @api private
 */
var _importUser = function(app, authenticationStrategy, forceProfileUpdate, user, callback) {
    var userProfile = {
        'displayName': user[0],
    ...
    if (authenticationStrategy === 'local') {
        identifier = user[1];
        credentials.password = user[2];
    } else if (authenticationStrategy === 'shibboleth') {
        credentials.shibbolethId = user[2];
        identifier = user[2];
    }

    UsersDAO.getUserByCredentials(app, authenticationStrategy, identifier, function(err, persistedUser) {
        if (err && err.code !== 404) {
            return callback(err);

        // If we get a 404, it means the user doesn't exist yet and we can safely create it
        } else if (err && err.code === 404) {
            return UsersDAO.createUser(app, userProfile, credentials, callback);

        // Otherwise the user exists
        } else {
            // If we don't force updates or there's nothing to update, we can move on to the next user
            if (!forceProfileUpdate || (persistedUser.displayName === user[0] && persistedUser.email === user[1])) {
                return callback();
            }
