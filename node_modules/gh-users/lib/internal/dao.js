...
/**
 * Initialise the users DAO. This will create indices for
 * the displayName, email and shibbolethId columns
 *
 * @param  {Function}       callback        Standard callback function
 * @param  {Object}         callback.err    An error object, if any
 */
var init = module.exports.init = function(callback) {
    // Enable index searching based on trigrams. This assumes that the trigram postgres extension
    // has been enabled. See http://www.postgresql.org/docs/9.1/static/pgtrgm.html for more information.
...
var updateUser = module.exports.updateUser = function(user, update, callback) {
    user.updateAttributes(update).complete(function(err, user) {
        if (err) {
            log().error({'err': err, 'user': user.id}, 'Failed to update a user');
            return callback({'code': 500, 'msg': err.message});
        }
 ...
 *
 * @param  {Number}         appId               The application to search users in
 * @param  {String}         filter              The string to filter users by
 ...
 */
var searchUsers = module.exports.searchUsers = function(appId, filter, limit, offset, callback) {
    limit = limit || 10;
    offset = offset || 0;
    ...
    query += 'FROM "Users" ';
    query += 'WHERE "AppId" = ? ';
    query += 'ORDER BY (similarity("displayName", ?) + similarity(coalesce("email", ?), ?) + similarity(coalesce("shibbolethId", ?), ?)) DESC, "displayName" ASC, "id" ASC ';
    query += 'LIMIT ? OFFSET ?' ;
    var coalescedValue = '';
    var parameters = [appId, filter, coalescedValue, filter, coalescedValue, filter, limit, offset];
    sequelize.query(query, DB.User, null, parameters).complete(function(err, users) {
        if (err) {
            log().error({'err': err, 'filter': filter}, 'Unable to search on users');
