 ...
 * @FormParam   {string}            password                The password of the user making the change
 * @Return      {void}
 */
var _changePassword = function(req, res) {
    UsersAPI.changePassword(req.ctx, req.params.id, req.body.newPassword, req.body.password, function(err) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
