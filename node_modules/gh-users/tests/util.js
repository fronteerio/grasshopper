...
var AppTestsUtil = require('gh-apps/tests/util');
 ...
 * @param  {Boolean}            [options.canViewAllFields]      If `true`, extra profile fields such as `email` and `shibbolethId` will be asserted
...
var assertUser = module.exports.assertUser = function(user, options) {
    ...
    _.each(user, function(val, key) {
        if (options.canViewAllFields) {
            assert.ok(_.contains(privateFields, key), key + ' is not allowed on a public user profile');
        } else {
            assert.ok(_.contains(publicFields, key), key + ' is not allowed on a user profile');
        }

        // Assert each field has the correct value
        if (options.expectedUser) {
            assert.strictEqual(val, options.expectedUser[key]);
        }
    });

    var fieldsToCheck = publicFields;
    if (options.canViewAllFields) {
        fieldsToCheck = privateFields;
    }
...
/**
 * Assert that a user's password can be changed
 *
 * @param  {RestClient}         client                          The REST client to make the request with
...
var assertChangePassword = module.exports.assertChangePassword = function(client, id, newPassword, password, callback) {
    ...
    client.user.changePassword(id, newPassword, password, function(err) {
        assert.ok(!err);

        // Grab the user's profile information so we can attempt a local login
        // and verify the password was changed successfully
        assertGetUser(client, id, function(user) {

            // Get the application the user belongs to
            TestsUtil.getGlobalAdminRestClient(function(globalAdminClient) {
                AppTestsUtil.assertGetApp(globalAdminClient, user.AppId, null, function(app) {

                    // Ensure that we can log in with the new password
                    TestsUtil.getAuthenticatedAppUserClient(app, user.email, newPassword, function(newClient) {
...
/**
 * Assert that a user's password can not be changed
 *
 * @param  {RestClient}         client                          The REST client to make the request with
 * @param  {Number}             id                              The id of user for which to change the local password
 * @param  {String}             newPassword                     The new password for the user
