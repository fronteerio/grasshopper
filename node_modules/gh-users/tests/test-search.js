            ...
            TestsUtil.generateTestTenant(1, function(tenant, app) {
                ...
                ConfigTestsUtil.assertUpdateConfig(globalAdminClient, app.id, defaultConfiguration, function() {
                    ...
                    AuthTestsUtil.assertShibbolethLogin(app, null, attributes, null, function(restClient) {
                        ...
                        // Get the shibboleth user
                        UsersTestsUtil.assertGetMe(restClient, function(shibUser) {
        ...
        UsersTestsUtil.assertGetUsers(actor.client, app, query, null, null, function(users) {
            _.each(users.results, function(user) {
                ...
                if (user.id === actor.profile.id || expectPersonalInfo) {
                    UsersTestsUtil.assertUser(user, {'canViewPrivateFields': true});
                } else {
                    UsersTestsUtil.assertUser(user, {'canViewPrivateFields': false});
                }
            });
            return callback();
        });
