...
describe('Users - calendars', function() {
    ...
    describe('JSON', function() {
        ...
        it('verify events order and metadata', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2013, 1, false, function(simon) {
                ...
                SeriesTestsUtil.generateSerieWithEvents(simon.client, 3, 4, start, end, function(series) {

                    // Subscribe to all the series
                    SeriesTestsUtil.assertSubscribeSeries(simon.client, series[0].id, simon.profile.id, null, function() {
                        SeriesTestsUtil.assertSubscribeSeries(simon.client, series[1].id, simon.profile.id, null, function() {
                            SeriesTestsUtil.assertSubscribeSeries(simon.client, series[2].id, simon.profile.id, null, function() {
                ...
                OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'Orgunit 1', 'course', null, function(orgUnit1) {
                    OrgUnitTestsUtil.assertCreateOrgUnit(global.tests.admins.cam2013.client, 'Orgunit 2', 'course', null, function(orgUnit2) {
                        ...
                        SeriesTestsUtil.generateSerieWithEvents(simon.client, 3, 4, start, end, function(series) {
                            ...
                            OrgUnitTestsUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, orgUnit1.id, [series[0].id], function() {
                                OrgUnitTestsUtil.assertAddOrgUnitSeries(global.tests.admins.cam2013.client, orgUnit2.id, [series[1].id], function() {

                                    // Subscribe to all the series
                                    SeriesTestsUtil.assertSubscribeSeries(simon.client, series[0].id, simon.profile.id, orgUnit1.id, function() {
                                        SeriesTestsUtil.assertSubscribeSeries(simon.client, series[1].id, simon.profile.id, orgUnit2.id, function() {
                                            SeriesTestsUtil.assertSubscribeSeries(simon.client, series[2].id, simon.profile.id, null, function() {
    ...
    describe('iCal', function() {
        ...
        it('verify the description when getting a timetable calendar', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2013, 2, false, function(simon, nico) {
                UsersTestsUtil.assertUpdateUser(nico.client, nico.profile.id, {'displayName': 'Nico'}, function() {

                    UsersTestsUtil.assertGetMe(simon.client, function(me) {
                        // Set up an event with lots of metadata
                        var calendarStart = moment().subtract(1, 'day').format();
                        var calendarEnd = moment().add(30, 'day').format();
                        SeriesTestsUtil.generateSerieWithEvents(simon.client, 1, 1, calendarStart, calendarEnd, function(series) {
                            var serie = series[0];
                                                ...
                                                checkICalEventDescription(simon, serie, ['Jill'], null, null, null, 'with Jill', function() {
                                                    checkICalEventDescription(simon, serie, ['Jill', 'Jack'], null, null, null, 'with Jack, Jill', function() {
                                                        checkICalEventDescription(simon, serie, ['Jack', 'Jill'], null, null, null, 'with Jack, Jill', function() {
                                                            checkICalEventDescription(simon, serie, ['Jack', 'Jill', nico.profile.id], null, null, null, 'with Jack, Jill, Nico', function() {
                                                                            ...
                                                                            checkICalEventDescription(simon, serie, ['Jill'], null, null, 'bring paper', 'with Jill\n\nbring paper', function() {
                                                                                checkICalEventDescription(simon, serie, ['Jill', 'Jack'], null, null, 'bring paper', 'with Jack, Jill\n\nbring paper', function() {
                                                                                    checkICalEventDescription(simon, serie, ['Jack', 'Jill'], null, null, 'bring paper', 'with Jack, Jill\n\nbring paper', function() {
                                                                                        checkICalEventDescription(simon, serie, ['Jack', 'Jill', nico.profile.id], null, null, 'bring paper', 'with Jack, Jill, Nico\n\nbring paper', function() {
                                                                                            ...
                                                                                            checkICalEventDescription(simon, serie, ['Jill'], 'Lab', null, 'bring paper', 'Lab with Jill\n\nbring paper', function() {
                                                                                                checkICalEventDescription(simon, serie, ['Jill', 'Jack'], 'Lab', null, 'bring paper', 'Lab with Jack, Jill\n\nbring paper', function() {
                                                                                                    checkICalEventDescription(simon, serie, ['Jack', 'Jill'], 'Lab', null, 'bring paper', 'Lab with Jack, Jill\n\nbring paper', function() {
                                                                                                        checkICalEventDescription(simon, serie, ['Jack', 'Jill', nico.profile.id], 'Lab', null, 'bring paper', 'Lab with Jack, Jill, Nico\n\nbring paper', function() {

                                                                                                            // Check an event with some organisers, a type, a description and some notes
                                                                                                            checkICalEventDescription(simon, serie, ['Jack', 'Jill', nico.profile.id], 'Lab', 'this is an awesome event that requires paper', 'bring paper', 'Lab with Jack, Jill, Nico\nthis is an awesome event that requires paper\n\nbring paper', function() {
                                                                                                                return callback();
                                                                                                            });
                                                                                                        });
                                                                                                    });
    ...
    describe('RSS', function() {
        ...
        /**
         * Test that verifies that data is escaped when exporting
         */
        it('verify data is escaped when exporting', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2013, 1, false, function(simon) {
                UsersTestsUtil.assertGetMe(simon.client, function(me) {
                    ...
                    SeriesTestsUtil.generateSerieWithEvents(simon.client, 1, 10, calendarStart, calendarEnd, function(series) {
                        ...
                        // Update the displayName and description of an event to something that contains characters that should be escaped
                        var update = {
                            'displayName': '</title><foo>The bar is strong</foo>',
                            'description': '</item>'
                        };
                        EventsTestsUtil.assertUpdateEvent(simon.client, series[0].events[0].id, update, function() {
                            series[0].events[0] = _.extend(series[0].events[0], update);

                            SeriesTestsUtil.assertSubscribeSeries(simon.client, series[0].id, simon.profile.id, null, function() {

                                // Verify the RSS feed contains the events
                                UsersTestsUtil.assertGetUserCalendarRss(simon.client, simon.profile.id, me.calendarToken, series[0].events, function(calendar) {

                                    // The utility will have parsed the RSS feed and unescaped the
                                    // characters. This allows us to do strict equality comparisons
                                    var item = _.find(calendar.item, {'title':  [update.displayName]});
                                    assert.deepEqual(item.title, [update.displayName]);
                                    assert.deepEqual(item.description, [update.description]);

                                    // Ensure that the `foo` element didn't leak through somehow
                                    assert.ok(!item.foo);
                                    assert.ok(!calendar.foo);
                                    return callback();
                                });
                            });
                        });
