...
describe('Users', function() {
    ...
    describe('Changing a user\'s password', function() {

        /**
         * Test that verifies that a user's password can be updated
         */
        it('verify a user\'s password can be updated', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2014, 1, false, function(simon) {

                UsersTestsUtil.assertChangePassword(simon.client, simon.profile.id, 'a new password', simon.client.options.password, function(newClient) {
                    return callback();
                });
            });
        });
        ...
        /**
         * Test that verifies validation when changing a user's password
         */
        it('verify validation when changing a user\'s password', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2014, 1, false, function(simon) {

                var newPassword = 'new password';
                var oldPassword = simon.client.options.password;

                // Invalid user id
                UsersTestsUtil.assertChangePasswordFails(simon.client, 'Not a number', newPassword, simon.client.options.password, 400, function() {
                    // Unknown user id
                    UsersTestsUtil.assertChangePasswordFails(simon.client, -1, newPassword, simon.client.options.password, 404, function() {
                        UsersTestsUtil.assertChangePasswordFails(simon.client, 23121312, newPassword, simon.client.options.password, 404, function() {

                            // Missing new password
                            UsersTestsUtil.assertChangePasswordFails(simon.client, simon.profile.id, null, simon.client.options.password, 400, function() {

                                // Too short new password
                                UsersTestsUtil.assertChangePasswordFails(simon.client, simon.profile.id, 's', simon.client.options.password, 400, function() {

                                    // Missing password for the current user
                                    UsersTestsUtil.assertChangePasswordFails(simon.client, simon.profile.id, newPassword, null, 400, function() {

                                        // Verify administrators need to provide their password as well
                                        UsersTestsUtil.assertChangePassword(global.tests.admins.cam2014.client, simon.profile.id, 'a new password', global.tests.admins.cam2014.client.options.password, function(newClient) {
                                            return callback();
                                        });
                                    });
                                });
        ...
        /**
         * Test that verifies authorization when changing a user's password
         */
        it('verify authorization when changing a user\'s password', function(callback) {
            TestsUtil.generateTestUsers(global.tests.apps.cam2014, 2, false, function(simon, nico) {

                var newPassword = 'new password';
                var oldPassword = simon.client.options.password;

                // Anonymous users can't change other user passwords
                TestsUtil.getAnonymousAppUserClient(global.tests.apps.cam2014, function(anonymousClient) {
                    UsersTestsUtil.assertChangePasswordFails(anonymousClient, simon.profile.id, newPassword, oldPassword, 401, function() {
                        TestsUtil.getAnonymousGlobalAdminRestClient(function(anonymousGlobalAdminClient) {
                            UsersTestsUtil.assertChangePasswordFails(anonymousGlobalAdminClient, simon.profile.id, newPassword, oldPassword, 401, function() {

                                // Regular users can't change other users their password
                                UsersTestsUtil.assertChangePasswordFails(nico.client, simon.profile.id, newPassword, oldPassword, 401, function() {

                                    // Application admins can't change users from other applications their password
                                    UsersTestsUtil.assertChangePasswordFails(global.tests.admins.oxford2013.client, simon.profile.id, newPassword, oldPassword, 401, function() {

                                        // Regular users have to provide the correct old password
                                        UsersTestsUtil.assertChangePasswordFails(simon.client, simon.profile.id, newPassword, 'wrong password', 401, function() {
                                            UsersTestsUtil.assertChangePassword(simon.client, simon.profile.id, newPassword, oldPassword, function() {

                                                // App admins can change users from their application their password
                                                UsersTestsUtil.assertChangePassword(global.tests.admins.cam2014.client, simon.profile.id, newPassword, null, function() {

                                                    // Global admins can change users their password
                                                    TestsUtil.getGlobalAdminRestClient(function(globalAdminClient) {
                                                        UsersTestsUtil.assertChangePassword(globalAdminClient, simon.profile.id, newPassword, null, function() {
                                                            return callback();
                                                        });
                                                    });
                                                });
        ...
        /**
         * Test that verifies that only users with a local password can be changed
         */
        it('verify only users with a local password can be changed', function(callback) {
            // Create a new application and enable Shibboleth on it
            TestsUtil.getGlobalAdminRestClient(function(globalAdminClient) {
                TestsUtil.generateTestTenant(1, function(tenant, app) {
                    ...
                    ConfigTestsUtil.assertUpdateConfig(globalAdminClient, app.id, defaultConfiguration, function() {
                        ...
                        var attributes = {
                            'email': TestsUtil.generateTestEmailAddress(),
                            'displayname': 'A glorious name'
                        };
                        AuthTestsUtil.assertShibbolethLogin(app, 'shibNewland', attributes, null, function(client) {

                            // Get the user's profile information so we have their id
                            UsersTestsUtil.assertGetMe(client, function(me) {

                                UsersTestsUtil.assertChangePasswordFails(client, me.id, 'newPassword', 'oldPassword', 400, function() {
                                    return callback();
                                });
                            });
                        });
