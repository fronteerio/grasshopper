...
var getTenants = module.exports.getTenants = function(callback) {
    DB.Tenant.all().complete(function(err, tenants) {
        if (err) {
...
var getTenant = module.exports.getTenant = function(id, callback) {
    DB.Tenant.find(id).complete(function(err, tenant) {
        if (err) {
            log().error({'err': err, 'id': id}, 'Failed to get a tenant');
            return callback({'code': 500, 'msg': err.message});
        } else if (!tenant) {
            log().error({'id': id}, 'A tenant with the specified id could not be found');
...
var createTenant = module.exports.createTenant = function(displayName, callback) {
    ...
    DB.Tenant.create(tenant).complete(function(err, tenant) {
        if (err) {
...
var updateTenant = module.exports.updateTenant = function(tenant, displayName, callback) {
    ...
    tenant.updateAttributes(update).complete(function(err, tenant) {
        if (err) {
            log().error({'err': err}, 'Failed to update a tenant');
            return callback({'code': 500, 'msg': err.message});
        }
