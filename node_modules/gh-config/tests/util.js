...
var assertUpdateConfig = module.exports.assertUpdateConfig = function(client, appId, update, callback) {
    ...
    /*!
     * Retrieve the config and assert the updates were persisted. The assertions will
     * only be made when the REST request finished and when the config has been recached
     */
    var done = function() {
        if (isRequestDone && isCached) {
            ...
            assertGetConfig(client, appId, false, function(config) {
                ...
                _.each(update, function(val, key) {
                    ...
                    assert.equal(config[key], val, 'Wrong value for ' + key);
                });
