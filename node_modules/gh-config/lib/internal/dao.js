...
var _setUpAppsTrigger = function(callback) {
    ...
    sequelize
        ...
        .complete(function(err) {
            if (err) {
                log().error({'err': err}, 'Unable to create config trigger function');
                return callback({'code': 500, 'msg': 'Unable to create config trigger function'});
            }

            return sequelize.query(triggerQuery);
        }).complete(function(err) {
            if (err) {
                log().error({'err': err}, 'Unable to create config trigger');
                return callback({'code': 500, 'msg': 'Unable to create config trigger'});
            }

            log().debug('Created config trigger');
            return callback();
        });
};
 ...
 * @param  {Object}     update              The updated configuration values
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error that occurred, if any
 */
