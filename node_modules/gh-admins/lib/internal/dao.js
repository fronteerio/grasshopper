...
var getGlobalAdmin = module.exports.getGlobalAdmin = function(id, callback) {
    DB.GlobalAdmin.find(id).complete(function(err, globalAdmin) {
        if (err) {
...
var getGlobalAdminByUsername = module.exports.getGlobalAdminByUsername = function(username, callback) {
    DB.GlobalAdmin.find({'where': {'username': username}}).complete(function(err, globalAdmin) {
        if (err) {
...
var createGlobalAdmin = module.exports.createGlobalAdmin = function(username, password, displayName, callback) {
    ...
    AuthUtil.hashPassword(password, function(err, hashedPassword) {
        if (err) {
            log().error({'err': err}, 'Failed to hash the password when creating a new global administrator');
            return callback(err);
        }
 ...
 *
 * @param  {Number}             globalAdmin                 The global administrator to update
